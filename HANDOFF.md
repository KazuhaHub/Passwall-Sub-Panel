...
**节点问题降噪（v4.0.0-beta.8，2026-09-14）**：在现有页面及批量操作基础上增强，不新增独立页面。
默认重要问题；诊断／全部记录以可见 Tabs 切换，切换清空勾选并回到第一页。后端 `view` 筛选
在计数和分页前执行；未传／all 保持旧接口行为，无效值拒绝。仅两个核验过的精确启动记录
（core_telemetry_failed + collect core telemetry: xray／sing-box process is starting）降为诊断；
未知错误、degraded、旧超时及任务安全问题仍保留关注，不依据时间或历史上报猜测恢复。
显示按 agent + 统计／同步／任务安全／其他类别合并；主列表提供简短处理建议，不展示原始
代码、错误或截断 Agent ID。详情先显示紧凑的原始记录选择／查看状态／确认按钮，原代码、
完整身份、对象、日志和时间默认折叠；混合代码分组仍逐条保留各自原文。批量确认始终按
选中的原始 ID，不扩展到诊断列表、另一页或新到记录。不删除、不自动确认、不修改 Node 协议
或 schema。前端全量 527 项测试／51 个文件、最终页面 42 项回归、生产构建、全仓 Go 测试与
go vet 通过；本地隔离浏览器验证分类切换、选择清空、合并记录批量选择和折叠原日志。
仅内存假数据，未操作线上记录；纳入 beta8 发行。

**节点问题批量操作（v4.0.0-beta.8，2026-09-14）**：支持勾选本页问题组、全选本页待查看原始记录，
也可在详情逐条勾选；批量确认前明确显示原始记录数量，不跨页选择。复用逐条确认接口，
默认最多 8 个并发，显示完成进度并汇总成功／失败数量；失败记录重新读取后仍待查看且在
本页时保留勾选供重试，不伪造确认时间或把已查看当作已恢复。分页、筛选、搜索和手动刷新
清空选择；同步操作锁防止重复确认，确认后再次检查权限。未新增批量后端路由、schema、
删除或修复能力。前端全量 515 项测试／50 个文件与生产构建通过；隔离浏览器验证取消保留
选择、实际批量确认两条内存记录及 390px 窄屏选择栏换行，无线上写入；纳入 beta8 发行。

**通知类型误判修复（v4.0.0-beta.8，2026-09-14）**：beta7 通知中心对所有后端的 PanelVersion 调用
3X-UI 专用升级比较，导致 Passwall Node `v0.0.1-beta4 (4b40af2)` 被提示可升级至 3X-UI 3.7.0。
现在仅规范化类型为 3xui 的服务器进入该回调；空类型旧记录仍按 3X-UI，Passwall Node／S-UI／
未知类型排除，不通过版本字符串猜测类型。真正的 3X-UI 更新和其他类别通知保留，铃铛计数
由修正后的 feed 重新计算。通知实时派生，无需删除通知记录、迁移数据库、重装或修改节点配置。
服务与 HTTP 回归覆盖原截图、混合后端、旧类型兼容及管理员／操作员过滤计数；修复前复现失败、
修复后对应测试通过。本次不增加 Node／S-UI 的铃铛升级类别，版本列提示同样纳入 beta8。

**节点问题页面简化初版（v4.0.0-beta.8，2026-09-14，现由上述降噪增强）**：初版按当前页的 agent + 稳定问题代码分组，
仅展示易读摘要、服务器名称、原始记录数、最近出现和详情入口；移除重复的大块红色状态和
长 Agent ID。分组不修改原始 issue ID、持久化去重、分页总数或确认语义，数量明确限于本页。
详情保留完整代码、Agent ID、对象、错误原文及首次／最近／确认时间，仍逐条确认已查看；
已查看不代表已恢复，也不是实时健康检查。服务器名称由当前页一次批量只读查询补充，不读取
或解密连接凭据；元数据失败仍保留原记录，搜索同时支持服务器名称。窄屏使用摘要卡片，
请求取消避免旧筛选覆盖新结果，确认末页最后一条记录后自动回到有效页。
前端 490 项测试、前端生产构建、全仓 Go 测试与 go vet 通过；隔离浏览器模拟数据验证中文／
英文、桌面／390px 窄屏及逐条确认，不修改线上记录；纳入 beta8 发行。

**服务器操作图标与更新提示（v4.0.0-beta.8，2026-09-14）**：安装／重装使用 DownloadOutlined，所有
节点／面板／内核升级使用配套带托盘的 UploadOutlined，英文入口为 “Install / Reinstall”。
Passwall Node 版本列读取已有已审核、已发布目录，严格按服务器 stable→stable、beta→testing
频道匹配与 SemVer 大小比较；点击提示仅打开原升级确认弹窗，不自动下发。目录失败显示检查
失败，未知／dev／未上报身份不猜测。S-UI 提示官方稳定版并链接到官方发行页面；不增加其
不具备的远程升级能力。S-UI 首次无缓存时单独等待共享版本元数据，不阻塞服务器列表／连接
测试；元数据和连接探测完成顺序任意，版本提示仍按最新上报版本重算。仅元数据与界面改动，
不改变固定凭据、节点配置或任务协议。前端 455 项测试、前端生产构建、全仓 Go 测试与
go vet 均通过；S-UI 版本查询 race 测试通过；纳入 beta8 发行。

**UDP / QUIC 默认策略调整（v4.0.0-beta.8，2026-09-14）**：UDP 默认 DIRECT（允许 UDP、使用本地出口，
不是 REJECT 或禁用节点 UDP），QUIC 默认继续委托 UDP。默认组显示顺序为节点选择、UDP、QUIC；
规则顺序保持局域网直连、QUIC UDP/443、普通 UDP，避免普通 UDP 遮住 QUIC 独立覆盖。
两组均保留手动代理／直连／拒绝，Mihomo 与 sing-box 共用默认成员。只升级已知官方默认规则，
管理员自定义组成员／排序／文件与客户端已保存的选择不强制重置；已有客户端仍选代理时需要
手动将 UDP 组切到 DIRECT。此改动不更改节点协议、Flow 或底层代理的 UDP 传输能力。

**原生配置确认修复（v4.0.0-beta.7，2026-09-14）**：完整配置回执只有通过既有 epoch／ETag／
ObjectApplied 校验、且监听器属于已铸造文档时，才尝试确认节点配置；仓储事务锁定原服务器的
节点行，再核对完整 PSP-owned 监听意图，避免校验后发生的同时间戳配置／启用状态编辑被旧回执
误清。确认仅写实际端点、synced 与空 pending age，不重写配置、密文、捕获时间、健康或流量。
铸造与回执守卫共用 domain.NodeConfigIntent，保持原十字段 JSON 字节形状与 A/B/A 同内容确认
语义；不更改协议、模块依赖、schema 或内核版本。成功变更后统一清理 group 节点缓存与最终
render 缓存，重复确认不清缓存。旧状态可由下一次有效完整回执自动收敛，不需要管理员重装。
普通／失败／部分／陈旧回执、并发编辑、存储失败后重试、整行／密文保留和真实 Build 两层缓存
回归均有测试；发布仍先通过 PR 与合并 SHA 的现有 CI／Linux 安装验收闸，不操作线上部署。

**新增服务器表单简化（v4.0.0-beta.6，2026-09-12）**：Passwall Node 新增主表单仅保留完整名称的后端类型、
必填服务器名称、安装方式及默认更新频道；移除步骤条、重复的连接原理和安装教程。可选备注与技术
说明默认折叠；主表单明确说明先创建记录、下一页才生成命令，inbound 仍在节点页面单独配置。
默认仍为 Passwall Node + Linux 二进制／systemd + Stable；手动安装的平台／架构选择、编辑时的
频道保存、第三方面板真实管理地址／认证与字段校验不变。隔离页面预览只在内存创建假服务器，
等待状态不冒充节点已安装；既有 Linux 发布闸的禁止新建／禁止轮换断言不因演示扩展而放宽。
本版同时修复 inbound 编辑误报；开发与发行不修改线上数据。

**入站编辑修复（v4.0.0-beta.6）**：管理员读取已捕获的 PSP-owned 配置意图不再依赖同步已完成或
agent 在线；待同步、失败和漂移仍可编辑，附简短同步提示，但不把这次读取视为运行时就绪。
订阅渲染仍使用原来的严格收敛判据。未捕获的旧节点仍实时读取，失败显示可重试错误而非协议
不支持；真实未知协议／非 SS-2022 方法没有保存入口，防止旧 Shadowsocks 被默认改成 VLESS。

**安装体验简化（v4.0.0-beta.6，2026-09-12）**：安装／重装默认只展示安装方式、明确选择的
审核版本、一行短命令、复制反馈与当前状态／下一步。身份／长期凭据／原始私有脚本与发布说明
默认折叠；迁移阻塞、必要同意及重装前停止旧实例的提醒仍可见。原记录转换为 Passwall Node 后
刷新真实服务器类型，转入同身份安装入口，不再次申请迁移。普通安装、转换和旧 3X-UI 注册共用
完整下载后解析再执行的短命令；不流式执行网络响应，不暴露长期凭据到命令参数／环境。
Passwall-Node 配套安装器新增六阶段反馈、TTY 下载进度和有时限的进程启动检查，未声称代理就绪。
本版正式依赖锁定 `v0.0.1-beta4`，审核目录保留 beta3 精确重装并新增 beta4。
跨仓发行顺序仍须先发布／核验 PN，再更新 PSP 依赖及审核目录，通过真实 Linux 发布闸后发布 PSP；
验收版本与精确发布构建标识绑定，依赖／目录变更也触发该闸，不能把本地源码验证误当作正式模块证明。

**统一安装／重新安装与节点端命令（2026-09-12，v4.0.0-beta.5）**：更多菜单统一入口可选择
...
没有放宽 timeout 或宣称本机通过；GitHub Linux 生产页面冒烟已通过。
本版新增两个不可跳过的发布闸：真实 Linux Chromium 使用 built SPA 与隔离 API 验证原后端默认、
手动配置原 ID、固定身份、精确版本确认及节点命令复制；仅一次性 GitHub-hosted Ubuntu 24.04
VM 上运行真实 systemd／已发布 PN beta3／Xray VLESS 请求、同身份原地重装与节点数据丢失后重装。
旧 x-ui 仅是明确标注的标准 systemd／SQLite 替身，不冒充真实第三方面板或生产恢复演练。
成功证据必须是实际运行并 PASS（不能 SKIP）；PR 与合并后发布 SHA 都通过才可发布。
脚本、限制及证据说明见 `tests/acceptance/node-reinstall-systemd/README.md` 与 PR #54。
