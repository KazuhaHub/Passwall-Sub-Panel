...
**新增服务器表单简化（未发布，2026-09-12）**：Passwall Node 新增主表单仅保留完整名称的后端类型、
...
此改动不包含上一轮诊断的 inbound 编辑误报修复，也没有发布或修改线上数据。

**安装体验简化（开发完成，未发布，2026-09-12）**：安装／重装默认只展示安装方式、明确选择的
...
PSP 的正式依赖仍锁定已发布的 `v0.0.1-beta3`；新安装器源码在 PN 同名功能分支。
后续须先发布／核验新 PN 版本，再更新 PSP 依赖及审核目录，重新通过真实 Linux 发布闸后发布 PSP；
不要将本地两源码工作区验证误当成已发布模块或本版真实 systemd 验收。
