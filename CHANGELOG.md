...
- **流量配额迟滞带（数据面演进计划 Phase 1a）** —— 详见新增的 [`docs/traffic-quota-deadband.md`](docs/traffic-quota-deadband.md)。修好量纲不匹配后，单客户端用户的推送值已经恒定（skip 命中率 98%）；**多客户端用户不成立**：`QuotaHeadroom` 是用户级的，被该用户所有客户端的流量一起拉低，而每个客户端只重基到自己的计数器上。于是客户端 A 什么都没干、只因为 B 烧了流量，A 的目标 cap 就下降——每周期每客户端一次全量替换，每次重启该节点的 core。这个漂移**不是 bug**（共享配额本就该这样），要消除的是写入不是语义。
  ...
  **`band = min(headroom / 20, 8 GiB)`** —— 锚在「剩余」而非「上限」上，于是带来一个与用户速率、套餐大小、使用模式都无关的不变量：**PSP 停止运行时，用户最多多用 5% 的时间**（剩余额度就是剩下的时间）。越接近耗尽 band 越小，需要精确的时候最精确。反过来锚在上限上是危险的：1 TB 套餐的 1% 是 10 GB，对只剩 1 GB 的用户就是授权超发十倍。不取更小的值是因为面板 cap 本就落后一个轮询周期的用户流量，**比这更小的 band 既不增加风险也压不下写入**——1% 实测只把 P=2 救到 47%，5% 救到 84%。8 GiB 绝对上限是唯一一处故意放弃比例论证的地方：多 TB 套餐上 5% 是一张带宽账单，且没人读到「5%」时想到的是 500 GB。
  ...
  **实测**（50 周期压力模型，每客户端连续维持 3.6–25 Mbps）：P=1 98%（不变）、**P=2 0% → 84%**、**P=3 0% → 65%**。真实机群应当好于此表。回归测试把 P=2/P=3 的下限钉在 70%/50%。
