...
**一个执法漏洞，加上第一个能让你自己看出「面板到底在不在干活」的页面。**

配额执法这次漏的是：一个**没有 snapshot 行**的用户，其周期用量被读成 0，于是 PSP 推给面板的保护上限等于**把整份配额又发了一遍**——而这个上限唯一的作用窗口，正是 PSP 自己挂掉、无法轮询的时候。没有 snapshot 行不是什么罕见状态：保留期裁剪会产生它，v2 导入的用户天生就是。
