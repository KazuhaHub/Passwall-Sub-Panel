...
- **`docs/observability.md` 的测量流程用错了鉴权方式** —— 写的是 `curl -b cookies.txt`，而 SPA 把 JWT 存在 `localStorage.psp_access` 里、以 `Authorization: Bearer` 发出；HttpOnly cookie 那条路只有 SAML ACS 会设。照文档做会得到 `401 {"error":"Missing token"}`。改为浏览器控制台的 `fetch` 写法。

- **重启自检缺了一个前提，会误报** —— 文档说 `uptime_ms` 与 `window_ms` 相差无几即中途重启过、样本作废。但若在刚启动时就 reset，这两个数从一开始就贴在一起、之后一直贴在一起，自检**分不清真重启和同时起步**。补上「先跑 20–30 分钟再开窗」，那段间隔本身就是「没重启过」的正面证据。
