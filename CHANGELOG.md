...
**这一版的主题是「写入成功了，但什么都没发生」——四处，各自独立，形状相同。**
...
- **编辑分隔标题不再抹掉它的创建时间。** 编辑路径从请求 DTO 构造实体（其中没有这个字段），而 `Save` 是整行写——于是每一次编辑都把该列盖成 Go 的零值时间，行读回来声称自己创建于公元 1 年。没有报错，没有告警，值就没了。这一列按定义不可变，所以现在让写入**根本碰不到它**,而不是小心地把旧值搬过去。（在复现上一条时撞到的。）
