...
## v3.9.2-beta.10 — 2026-09-01

把额度管理从「逐个用户」搬到分组（OU）：流量配额与两个连接上限现在默认继承所属分组，用户可逐字段覆盖。外加一个把「设了却不会生效的上限」当场说出来的界面提示，以及 PR #14 那批前端改动的发布说明补录。

### 新增

- **流量配额与连接上限现在从分组（OU）继承** —— 详见 [`docs/connection-limits.md`](docs/connection-limits.md) §10。三个额度原本只在用户行上，改一次机队策略就是 N 行编辑。现在默认继承所属分组，用户可逐字段覆盖。

  **难点不是继承，是 `0` 已经被占用了。** 它在两端都表示「不限」——PSP 这样编码，面板也这样读——所以分组一旦能承载策略，存量的 `0` 就是歧义的：不限，还是听分组的？读错就是给本来没配额的用户凭空造出配额，即流量下限缺陷换个位置重演。于是有了真正的第三态：`NULL` 继承、`0` 明确不限、`N` 明确为 N，三个字段各自独立解析。

  **存量 `0` 迁移为 `NULL`，只迁一次。** 之所以安全，理由很具体：在此之前 `0` 是表达「不限」的**唯一**方式，且**没有分组层可供对比**，所以没有任何一个存量 `0` 能携带「不限，且刻意不听分组的」这种意图——它没有丢弃任何曾经存在过的信息。若保留为明确 `0`，每个存量用户都会静默退出分组策略，运维方设了分组配额会发现对谁都不生效，正是这个功能要消除的逐人操作。落地瞬间行为不变：分组新列都是 `NULL`，继承解析为不限，正是 `0` 原本的含义。

  **新增 `schema_migrations` 表**，专为这一类迁移而设。`EnsureSchema` 里既有的回填都是幂等的（把 NULL 归并成本来就该是的 0），而**重新解释**一个值的迁移只正确一次——重跑会把运维方后来刻意填的 `0` 又变回继承。标记与数据改动写在**同一个事务**里，崩溃不会留下「数据改了、标记没写」的可重跑状态。

  **生效值与存储值分开放**：`User` 上那三个字段是解析后的生效值（约 100 个消费点只想要那个数字，计量循环不该被迫先拿到分组），存储的覆盖值在 `u.Limits`。这么切**安全的前提**是写路径为稀疏指针 DTO——请求里没提到的字段就不写——所以一次只改显示名的编辑不会把继承中的用户悄悄钉死在他分组当时的值上。该不变量有测试，并已变异验证。「恢复继承」需要独立信号，故用户侧有 `inherit_*`、分组侧有 `clear_*` 标志，形状与既有 `ClearExpire` 一致。创建路径携带标量、无法表达继承，那里的 `0` 读作「没有意见」，与迁移同源。

  **解析在用户仓储里做**，配 30s TTL 的分组限额缓存，分组写入即失效。每个已鉴权请求都会加载用户，按次查库不可接受；而分组策略是长期决定、作用于它的东西（多分钟轮询、每小时邮件）都更慢，30s 陈旧度无感。编辑分组仍然立即生效——两个仓储共用同一个缓存实例。读不到时向「不限」失败：因数据库抖动切断付费用户是更糟的错误，何况 PSP 自己仍在计量并停用。

  **界面**：分组「策略」页三个输入框，**留空 = 本组不作规定，填 0 = 明确不限**；用户编辑每个额度下方一个「跟随分组」勾选框，勾上时输入框置灰但**仍显示继承到的数值**——让人看见自己继承的是什么，而不是一个空框。

  **自查发现并修掉的三个缺陷**（`/code-review`，全是本轮引入）：**抬高配额不再解除流量停用**——`UpdateProfile` 写的是覆盖值，而旁边的解析值仍是编辑前的，`TrafficExceeded()` 拿旧配额判断，用户仍被停用、编辑看起来毫无作用（解析已移到保存后、第一个读取者之前）；**创建分组漏掉校验**——负数上限会被全组继承、原样推给面板，且连能力缺口告警都不触发（那个告警只在上限为正时才算缺口），而同样的值在用户端一直是 400；**改分组的连接上限从不推送到面板**——只有标签过滤变更会触发成员重推，闲置成员会无限期保留旧上限（轮询只碰有流量的用户）。第三条尤其值得记：我当时验证的是「分组变更到达成员的**解析值**」，那确实成立，但**推送到面板**是另一回事，而文档写成了已验证。

  **实机验证**：真实数据库 + 真实面板跑通完整生命周期——继承 → 改分组策略立即到达成员 → 单字段覆盖 → **无关编辑不钉死其余字段** → 分组再变、继承字段跟随而覆盖保持 → 交还给分组 → 清空分组策略回到不限。浏览器实测两个对话框。顺带修掉一个实机才发现的问题：建号响应返回的是**未解析**的 0，管理员建完看到 0/0/0、刷新才变成继承值。
...
- **自助注册改用分组选择器** —— 原先要手填分组，现在是选择器（PR #14，`5d76754`）。

- **TLS 加密套件选择器**（PR #14 及其后续修正）—— 新增 `TLSCipherSuitesSelect` 组件，不安全的套件会被显著标注为危险项。后续修正：修正套件配置本身（`2adb7b4`）、保持选择器高度稳定避免布局跳动（`958b307`）、危险项高亮（`8b63d83`）。

  **随后收窄为仅 3X-UI 可见**（`c7e8ab6`）：`buildTLSSettings` 的输出对两种面板都发，但 S-UI 从不消费 `cipherSuites`——`nodespec` 的 `decodeTLS` 不解析这个键，S-UI 适配器自建 TLS 块。而节点回读走 PSP 本地快照，所以重开编辑器仍会显示已勾选的套件，**管理员看到的是一条面板从未生效的加固策略**。`docs/panel-adapters.md` 对这种情况的约定是「编辑器隐藏，或适配器报错拒绝」。同时把该字段加进切换到非 3X-UI 面板时的重置列表。

### 修复

- **REALITY `minClientVer` 建议值回退到 `1.0.0`**（`be6e957`）—— PR #14 把建议值改成 `1.8.2`，但表单预填、placeholder 和两段注释都还是 `1.0.0`，且 `docs/3xui-compat.md` 2026-07-13 那次线上事故实际验证的修复也是 `1.0.0`。xtls/reality 的闸门是 `ClientVer >= MinClientVer`（闭区间），填 `1.8.2` 等于把余量压到零：任何自报低于该版本的客户端会被**静默丢到伪装站且服务端不产生日志**。S-UI 更直接——只放行空值或字面量 `1.0.0`，`1.8.2` 会让 REALITY 入站保存失败，而切换面板时表单并不重置该字段。

- **SSO 分组选择器分页**（`c7e8ab6`）—— 分组多时选择器不再一次性拉全量。

> 以上六条来自 PR #14 及其后续修正，非本轮工作；条目按提交信息如实整理，此前发布说明里缺失。
...
- **Phase 0 完成：40.9 小时生产窗口** —— v3.9.2-beta.8，25 用户 / 9 节点 / 5 面板，轮询 120s，`uptime 41.2h / window 40.9h`（未重启，样本有效）。详见 [`docs/data-plane-plan.md`](docs/data-plane-plan.md) 的「Phase 0 结果」。

  **计划 §1.2 的断言干净成立**：29296 次比较里 19358 次写入，写入原因 **100% 是 `total_gb`**——其余八个字段在 40.9 小时里**一次都没有**挫败过 skip。迟滞带打的靶子完全正确，不存在「别的原因大量出现所以白做」的分支。

  **N** p50 5.86（总 25，活跃占比 ~23%）、**P** mean **4.0**、**RTT** `GetClient` p50 204ms / `UpdateClient` p50 297ms。一轮 poll p50 809ms，其中 **`panel_fetch` 独占 655ms**，所有 DB 阶段亚毫秒——§1.1「读侧便宜」成立，但便宜的是 DB，成本全在网络。`pushSem` 会打满（inflight 峰值 = cap 8）但**排队峰值只有 1、跨周期结转 0**，§1.4 担心的雪崩在这个规模没有发生。

- **Phase 1a 验收：44.5 小时生产窗口（v3.9.2-beta.9，带 band）** —— `uptime 45.2h / window 44.5h / gap 38min`，未重启。

  ```
  total=35355  skipped=35252  write=103  err=0     → skip 命中率 99.7%（beta.8 同口径 33.9%）
  psp_lifecycle_quota_band_skip_total = 28201
  write reasons: total_gb=103                       → 仍然只有这一个原因
  carryover=0
  ```

  **写入从 473.3 次/小时降到 2.3 次/小时——204 倍。** 每次写入都是一个 `UpdateClient` 加一次该节点的 xray core 重启，按旧速率折算，这 44.5 小时省掉约 21000 次重启。poll p95 从 2236ms 降到 1887ms。

  **归因是干净的**：35355 次比较里 **79.8% 由 band 吸收**（确实存在配额差异）、19.9% 本就精确相等（delta=0）、0.29% 写入。skip 率的跃升不可能是「用户变安静」——`band_skip_total` 直接把它钉在迟滞带上。（注意 `band_skip` ⊆ `skipped` 只在本窗口成立：该计数器在「配额差异被吸收但同一次比较因别的字段仍要写」时照样加一，本窗口恰好没有出现别的写入原因。）

  实测 99.7% 略好于按分位数重建的投影 99.0%。

- **⚠️ 这个直方图在 band 上线前后量的不是同一件事** —— `psp_lifecycle_quota_delta_bytes` 的表达式一字未改，但它的左操作数是「PSP 上次写入在面板上留下的值」，所以它的含义由**当时生效的写入策略**共同决定。无 band 时几乎每次比较都写，它量的是**一个轮询周期**的兄弟漂移；有 band 时 skip 把旧值留在面板上，它变成**自上次写入以来累积**的漂移（本窗口平均每 343 次比较才写一次）。

  因此两个窗口的分位数**不可直接比较**：p50 从 7.0 KiB 变成 93.5 MiB、p95 从 14.9 MiB 变成 917.7 MiB、精确相等的比例从 33.9% 降到 19.9%，全部是迟滞的预期后果，不是用户流量涨了。max 2.29 GiB 超过 1 GiB 的 band 上限也不矛盾——那正是 band **正确放行**的那些写入。

  一般性结论，值得记住：**这个指标不能用来评价产生它的那个策略**。验收信号是 `band_skip_total`，不是 delta 分布。

- **Phase 2 决策门：不自研数据面** —— 代入实测数，推送侧容量为 8 并发 × (120s ÷ 721ms) ≈ 1332 次扇出/周期，当前占用 5.93，即容量的 **0.45%**；**余量 225×，约 5600 用户才饱和**。即使 25 个用户全部同时活跃也只用掉 1.9%。读侧随面板数扩展，约 300 个面板才填满一个周期。

  自研数据面（以及依赖它的 Phase 3 Agent）**要解决的瓶颈在这个规模下不存在**。重开此门的触发条件已写死在计划里（结转持续增长 / poll p95 超过周期的一半 / 用户数进四位数），避免日后凭感觉重启这个话题。Phase 3 全文保留但标注未触发。
...
- **流量配额迟滞带（数据面演进计划 Phase 1a）** —— 详见新增的 [`docs/traffic-quota-deadband.md`](docs/traffic-quota-deadband.md)。修好量纲不匹配后，单客户端用户的推送值已经恒定（skip 命中率 98%）；**多客户端用户不成立**：`QuotaHeadroom` 是用户级的，被该用户所有客户端的流量一起拉低，而每个客户端只重基到自己的计数器上。于是客户端 A 什么都没干、只因为 B 烧了流量，A 的目标 cap 就下降——每周期每客户端一次全量替换，每次重启该节点的 core。这个漂移**不是 bug**（共享配额本就该这样），要消除的是写入不是语义。

  **band 宽度是策略量，不是从数据里拟合出来的。** 一条很容易走上的岔路是取 `quota_delta_bytes` 的 P95 当 band——那是拿观测到的流量反推安全边界，把写入量当目标、安全裕度当副产品。正确的问法只有一个：**PSP 停止运行时，用户最多可以多用多少？** 因为 PSP 自己每轮询周期仍在精确的点切断，面板 cap 偏高只在 PSP 不工作时才兑现，而那正是面板侧 cap 唯一存在的理由。

  **`band = min(headroom / 20, 1 GiB)`** —— 锚在「剩余」而非「上限」上，于是带来一个与用户速率、套餐大小、使用模式都无关的不变量：**PSP 停止运行时，用户最多多用 5% 的时间**（剩余额度就是剩下的时间）。越接近耗尽 band 越小，需要精确的时候最精确。反过来锚在上限上是危险的：1 TB 套餐的 1% 是 10 GB，对只剩 1 GB 的用户就是授权超发十倍。不取更小的值是因为面板 cap 本就落后一个轮询周期的用户流量，**比这更小的 band 既不增加风险也压不下写入**——压力模型下 1% 只把 P=2 救到 47%，5% 救到 84%（**这两个数测于 8 GiB 上限；换成实际发布的 1 GiB 后同一模型是 57%**，换个上限掉 27 个点，说明在那个模型里起决定作用的是绝对上限而非百分比）。绝对上限是唯一一处故意放弃比例论证的地方：多 TB 套餐上 5% 是一张带宽账单，且没人读到「5%」时想到的是 500 GB。**而这个上限和百分比不同，它应当由实测决定**——它的职责就是给「band 能吞下的绝对漂移」封顶，而多大够用是经验问题。最初拍的 8 GiB 是在任何生产窗口存在之前定的；实测最大漂移 607 MiB，8 GiB 是它的 13 倍，白留了一个数量级，现已收到 **1 GiB**（仍覆盖住最坏周期，最坏超发量降为 1/8）。

  **不对称，这是关键。** 只容忍面板 cap **偏松**（有界，且只在 PSP 停摆时兑现，也正是稳态实际漂移的方向）。**偏紧一个字节也立刻改**——那会提前切断付费用户，且正是新计费周期、运维方调高配额到达面板的路径。对称的 band 会让「新周期第一天用户还被上期余量卡着」成为可能状态，那不是效率问题，是用户拿不到已付费的服务。

  **两个边界必须精确**，由整数除法自然给出、不需特判：`headroom = 0`（不限）band 为 0，否则就是给没配额的用户凭空造配额，即流量下限缺陷的镜像；`headroom = 1`（`TrafficFloorBytes` 的「已耗尽」哨兵）band 也为 0，否则 band 就成了「花完配额还能接着用」的洞。

  **两条推送链路同一个判定** —— 共享客户端模型与遗留 per-node 模型的漂移机制完全相同，只修一边等于留一个会复发的孪生缺陷。

  **实测**：把上面那份 40.9 小时窗口的漂移分布（p50 **7.0 KiB**、p90 3.58 MiB、p95 14.9 MiB、max 607 MiB）重建成流量阶梯喂回去，P=2/3/4 的 skip 命中率均为 **99.0%**——同一个窗口里的 19358 次写入会变成约 200 次。**中位数 7 KiB**：一次全量替换加一次 core 重启，为的是追平 100 GB 配额上 7 KiB 的差异，这是迟滞带最有力的论据。（这是**投影**，按分位数重建而非重放真实轨迹；真正的验收数来自升级后的 `psp_lifecycle_quota_band_skip_total`。）

  另有一组 50 周期的**压力模型**（每客户端连续维持 3.6–25 Mbps）保留作对照：P=2 57%、P=3 29%。它是在任何部署被测量之前编出来的，事后看**在 p95 上比现实狠约 40 倍**——它的*典型*每周期漂移差不多等于真实世界的*最大值*。保留它是为了回答另一个问题：band 在极端负载下**退化得是否优雅**（退到 29–57%，而不是塌回 0%）。它不用来定参数，回归下限也因此设得很低。

  **验收指标 `psp_lifecycle_quota_band_skip_total`** —— `skipped_total` 单独上升**不能归因**（用户变安静也会让它涨），这个计数器说明上升是 band 带来的。计的是被吸收的漂移而非被省掉的调用：同一次比较里若有别的字段不同仍会写入。
...
- **新增可插拔面板适配器与 S-UI 支持** —— 服务器可选择 3X-UI 或 S-UI，后端会按面板能力执行客户端、入站、证书、健康检查和订阅渲染；S-UI 入站支持读取、创建与更新，管理端会隐藏目标面板不支持的操作。
- **补全 S-UI 协议与配置渲染** —— 统一节点规格转换，支持 S-UI 的 Shadowsocks、Trojan、VLESS、VMess、Hysteria2、TUIC 与 AnyTLS 等配置进入订阅，并补充共享客户端挂载、入站写入和跨面板兼容测试。

### 改进

- **统一账户状态与代理服务状态** —— 登录权限、面板访问、订阅可用性和代理启用状态改由同一个后端访问快照判定；管理员界面同时展示账户与代理服务状态，避免“账户停用”和“服务暂停”混用。此改动不新增字段、不执行数据库迁移，也不批量改写旧数据；历史过期、流量超限及空原因记录会在运行时兼容解释。
- **扩大自动化测试覆盖并减少重复开销** —— 新增前后端 API、权限、中间件、安全头、加密与状态矩阵测试，优化 keyed mutex、并发任务和前端批量请求路径；API 状态映射只计算一次访问快照，降低重复判断和跨时刻不一致。
