...
- 同台服务器的相关同步消息合并为易读类别并给出简短处理建议；未知错误不直接显示技术代码，详情中的原始日志与身份默认折叠，逐条及批量确认保持可用。

- 节点问题支持分组勾选、全选本页待查看记录及详情内逐条勾选，批量确认前显示原始记录数量；执行时提供进度和汇总结果，失败记录保留勾选供重试。
- 批量确认复用逐条确认接口并限制并发，不删除记录或把已查看当作已恢复；分页、筛选、搜索和刷新清空选择，防止误操作其他页面的数据。

- 修复 beta7 通知中心将 Passwall Node／S-UI 版本误判为可升级到 3X-UI：仅 3X-UI 服务器参与其版本比较，铃铛计数同步纠正；无需重新安装或修改节点配置。

- 简化节点问题页面：同一服务器的同类问题在当前页合并展示易读摘要、服务器名称、记录数和最近出现，完整技术信息移入详情；窄屏改为卡片布局。
- 原始上报记录及逐条“确认已查看”保持不变，不将已查看误作恢复；支持按服务器名称搜索，修复旧筛选请求覆盖和确认末页最后一条后页码失效的问题。

- 服务器安装／重装、节点／面板／内核升级操作统一为带托盘的配套图标，英文入口统一为 “Install / Reinstall”。
...
- 修复 Passwall Node 已应用配置后，节点仍显示“配置推送等待重试”：有效的完整配置回执确认当前意图后清除待同步状态，并更新实际端点。
- 确认前在事务内重新核对服务器绑定和完整监听配置，旧回执、失败／部分回执及并发编辑不能误清新修改；仅更新确认字段，保留原配置、加密凭据、捕获时间、健康和流量。
- 配置确认后同时清理节点列表缓存和订阅输出缓存，下一次订阅读取立即使用已确认配置；重复回执不反复清缓存。
- 不新增数据库迁移，不更改 Node 协议或内核版本，不发布 Passwall Node 新版本。升级和兼容边界见 [beta7 发行说明](docs/releases/v4.0.0-beta.7.md)。
...
- 新增服务器默认选择排在首位的 **Passwall Node**，统一使用完整名称；在同一个弹窗内选择安装方式，创建后直接进入对应安装步骤。
- 默认 Linux 二进制 + systemd 一键安装；支持 Docker Compose 和 Linux/macOS/Windows（amd64/arm64）手动前台安装。提供私密配置文件、凭据文件、固定版本下载校验及可复制的分步命令；安装方式切换不会重新创建服务器或轮换固定凭据。
- 安装材料仅通过管理员私密接口生成，禁止缓存；脚本及配置文件的凭据读取必须先成功记录不含秘密的审计。安装 Agent 与配置代理节点仍分开，手动及 Docker 模式明确说明各自的升级方式。

### 数据库迁移与 CLI

- 收敛 V4 历史迁移：在任何 schema 写入前只读验证最终 V3 语义基线；更旧或未完成 V3 转换的数据库明确拒绝，指引先成功启动 `v3.9.2` / `v3.9.2-beta.20`。
- 将必要的 V3→V4 状态、凭据和旧索引桥接集中到一次性流程，保留 beta1 已有迁移标记；完成后不重复运行旧桥接。删除 V3 separator 重解释与额度 `0→NULL` 转换，继续保留流量 NULL 修复和必要的上游 Ownership 收敛。
- 删除 V2→V3 专用 `internal/migrate`；旧 `psp migrate` 及未消费 positional 参数在配置加载/数据库初始化前拒绝启动。V2 升级使用冻结 V3 工具，V3→V4 正常启动自动迁移。
- 新装以专用初始化标记支持失败后重试；迁移失败不等于数据库自动回滚。已发布版本、标签和镜像保持不变。
- 限制 MySQL `AutoMigrate` 的冗余唯一索引自动删除，保留操作员自定义唯一索引，让旧应用索引的定义保护在桥接中生效；V4 部分转换失败时不再无条件提示在同一库运行 V3。

## v4.0.0-beta.1 — 2026-09-12

安装、远程升级及回退边界见 [beta1 发行说明](docs/releases/v4.0.0-beta.1.md)。以下修复已包含于 beta1 源码，不是 beta2 新增。
...
- **健康探测会把节点的端口改回去，而且是唯一能把它改成 0 的地方。** `UpdateHealth` 一直在写
  ...
  快照原样回声写回去。一趟的时长上界是 (节点数/并发) × 超时，所以一个几十秒前的值可以覆盖掉
  这期间 reconcile 或管理员刚写进去的端口。列作用域挡不住这件事——`UpdateInboundConfig` 写的
  **正是同一对列**,两个写者撞的是同一列而不是不同列。最坏路径是闭合的：一个老行的 port 被回填
  成 443 → health 用旧快照写回 0 → 下一轮判定为漂移 → 默认开启的反向推送把 `Port: 0` 推给面板，
  而 `specToRaw` 对此没有任何守卫。现在 health 只写探测结论，探测**目标**归 inbound 快照的写者所有。

  同时补上那道缺失的守卫：`SpecFromNode` 现在返回 `(spec, error)`,`Port <= 0` 一律拒绝。
  **改签名是刻意的**——四个推送点被编译器逼着处理它，而不是靠「记得调用校验器」,后者正是这个
  项目反复见到的、会失效的那类机制。一个节点端口为 0 是**合法的中间状态**（v3.5 之前的老行、
  刚导入还没回填的行），但把它**推出去**不是：那是拿一个坏掉的监听器换掉一个正在工作的。

- **UDP 节点的健康探测在结构上只会说「健康」。** Hysteria2 / TUIC 走 UDP，而旧实现向端口写一个 `0x00` 字节，然后把「没人回话」当成**开放**。没有任何 QUIC 服务器会回应那个字节，所以这个分支**永远走不到「不健康」**——主机整台没了、防火墙 DROP、进程崩了，写进库里的都是 `ok`。而且它写的是 `ok` 而不是「未知」,所以还会**盖掉**真正的未知状态。

  现在发的是一个 **QUIC 版本协商包**（RFC 9000 §17.2 长包头 + §15 保留版本号 `0x?a?a?a?a`,填充到 §14.1 规定的 1200 字节最小值）。这是 QUIC 里**唯一不需要密钥、不需要 ALPN、不需要有效连接 ID** 就能问出答案的交换，所以是我们能对一个代理服务器提出、而它会在无凭据情况下回答的唯一问题。三种结果全部是真实且互不相同的：

  | 观测 | 判定 |
  |---|---|
  | 收到回包 | **健康**——确实有 QUIC 服务器在监听 |
  | ICMP 端口不可达 | **不可达**——确实没有东西在监听 |
  | 沉默 | **无法判定**——过滤掉的端口、死掉的主机、以及**开了混淆的 Hysteria2**（它按设计就不该回应陌生人）从这里看完全同形 |

  第三种是新增的状态，**它绝不会被显示成绿色**。「探测不出来」和「一切正常」必须长得不一样，否则一支探测器坏掉的机群看起来就跟健康的一模一样。它也**不进告警流**：开了混淆的节点是永久不可探测的，一条永远在响、而且没人能消掉的告警，只会训练运维忽略整个通道。它有自己的颜色、自己的文案、以及仪表盘上自己的计数。

  **已对真实的 quic-go 服务器验证过**（Hysteria2 与 TUIC 都构建在它之上）：它对这个包回了 35 字节，版本字段为 `0x00000000`,即一个货真价实的版本协商包。所以未开混淆的健康节点仍然是绿的——没有这一步验证，这个改动可能会把整片 UDP 机群报成「无法判定」,而且看上去还很有原则。

- **探测器唯一会写的「不健康」状态，在管理端被显示成「尚未探测」。** 后端写的是 `unreachable`,而节点列表的调色板里没有这个键，于是落到 `?? palette['']`——灰点、文案「尚未探测」。仪表盘同样落到 default 灰。**一个已经挂掉的节点被显示成「还没测过」**,正是那段代码的注释里说这个状态就是为了避免的事情的镜像。类型联合、两个视图的调色板、以及中英两份文案里都缺这个键。

  更讽刺的是：调色板里**有颜色的那三个状态**（`panel_unreachable` / `inbound_missing` / `inbound_disabled`）自 v3.5 起**一个生产者都没有**——健康探测从那时起就完全不再调用 3X-UI。所以这张表给三个永不发生的状态配了色，却漏掉了唯一真正会发生的那个。

  颜色表因此被提取成 `nodeHealth.ts`,两个视图都从它取色，并由一个守卫断言：**后端能写出的每一个状态，都必须有自己的颜色和两种语言的文案**——落到「尚未探测」那个兜底值，就不只是没配样式，而是在对这个节点做一个不同且错误的陈述。
...
- 配额执法漏的是：一个**没有 snapshot 行**的用户，其周期用量被读成 0，于是 PSP 推给面板的保护上限等于**把整份配额又发了一遍**——而这个上限唯一的作用窗口，正是 PSP 自己挂掉、无法轮询的时候。没有 snapshot 行不是什么罕见状态：保留期裁剪会产生它，v2 导入的用户天生就是。
- **禁用一个节点或分隔标题，订阅还会继续发它最长一分钟**——数据库写对了，界面也没骗你，只是没人去清渲染缓存。
- **改一次流量轮询周期是静默无效的**,循环启动时抓走那个值就再也不看。
- **编辑分隔标题、或改 ACME 的 DNS 凭据，会把它的创建时间抹掉。**
- **新建节点 / 分隔标题 / 证书时把开关关掉，建出来是开着的**——GORM 把 Go 的 `false` 当成「没填」,套用了列默认值。其中「新建禁用节点」建出来是启用的，而启用的节点会直接进入每个人的订阅。
- **重试队列自己入队失败时，失败被吞成成功**——面板没更新、没有任何东西在排队，而调用方被告知一切正常。

> **升级后到「诊断」页看一眼**（左侧 报表 → 诊断，仅管理员可见）。第一次打开时窗口通常短于一个轮询周期，页面会**明确拒绝给数字**并告诉你还要等多久——这是刻意的：窗口不够长时的 0 不是健康的证据。等一两个周期后再看「结论」条。

### 修复

- **禁用节点 / 分隔标题现在立刻从订阅里消失。** 这条是用户报的：「禁用分隔标题，提示成功，但实际还是开启状态」。写入本身没问题——行里是 `enabled=false`,API 也这么返回。没发生的是**清掉已渲染的订阅缓存**,于是订阅继续把这个已关掉的行发出去，最长到缓存的 60 秒 TTL；而客户端自己还缓存一层，看起来就像永远没生效。实测：禁用前 19 处，禁用后**立刻仍是 19 处**,在 +30~45 秒之间才消失。

  **报上来的是里面轻的那个。** 此前**只有两条 reorder 路径**会清缓存，另外 15 个变更方法都不会——**`SetEnabled` 也在其中**。运维把一台节点下线（通常是有原因的），订阅还会继续把它发给每一个客户端一分钟。`DeleteAndSync`、`DetachAndSync`、`UpdateMetadata`、`UpdateInboundConfig`、`CreateInbound`、`ImportExisting` 同样一声不响。

  所以修法不是补那 15 处调用。**「记得调用失效器」正是失效了的那个机制**,补完这 15 个，陷阱原样留给第 16 个。现在它是**写入本身的属性**：本服务持有的仓储句柄被装饰，新增的变更方法不用知道这件事就自动带上。

  两点让它不是无脑全清：只包**运维拥有的写入**（连 `UpdateHealth` 和流量计数器也清的话，渲染缓存每分钟被清好几次、直接作废），而这条运维/轮询的分界线是这个包里那些 "column-scoped write" 注释一直在维护的同一条；并且只包**这个服务**的句柄，轮询与健康循环持有的仍是未装饰的仓储。

  留下来的是守卫：`ports.NodeRepo` 上每个写方法现在必须被归类——订阅可见，或写明理由归入轮询自有。新加一个而不做决定会直接测试失败，而不是默认落到导致这个缺陷的那个答案。

- **`bool` 列的默认值不再让「关掉的开关」存成开着的。** GORM 在列带 `default` 时会把零值字段整个从 INSERT 里省掉，于是 `bool` + `default:true` **存不下 `false`**——新建时把开关关掉，建出来是开着的。写入不报错，存进去的是相反的值。

  这个仓库已经被咬过一次（geo 的 `complete` 列，注释里记着：每个部分计数都声称自己是总数）。这次扫出另外三处，而且全是运维**刻意**去关的开关：**新建禁用的节点建出来是启用的**（而启用的节点直接进每个人的订阅，这一处真的会发流量）、新建的分隔标题开关关不住、新建证书时关掉的自动续期会自己打开。

  修法是行结构里改成 `*bool`（领域类型不动）：列默认值保留给 AutoMigrate 回填存量行，指针只是让 `false` 不再读作「没填」。加了一个类型级守卫——以后任何带非 `false` 默认值的 `bool` 字段都会在**声明列的那一刻**测试失败。

- **重试排不进队时不再报成功。** 七处调用点都是同一个写法：推送面板失败 → 入队重试 → **连入队都失败时，记一行日志、返回成功**。

  入队成功时返回成功是对的（活儿已经持久化排队，同步循环会收敛它）。**只有入队本身失败时是错的**：此时面板没被更新、没有任何东西在排队等着修它，而调用方被告知成功。「一行日志不是一次重试」——这句话 beta.17 的说明里已经写过一次。

  在用户相关的路径上最要命：PSP 自己的行已提交，所以用户从后续的订阅拉取里消失了；但**真正拒绝连接的是面板侧的客户端**,而已经配置好的客户端在那一步落地前照常能用。于是账号看起来被停了、实际没有，而且不会自愈。

  实际触发面窄（入队是本地 DB 写，主要在数据库恰好在两次写之间不可用时失败），但代价是静默且无上限，所以还是修了。返回的错误会说明下一步该做什么——本地已经写好、操作幂等，重做一次即可收敛。

- **编辑分隔标题不再抹掉它的创建时间。** 编辑路径从请求 DTO 构造实体（其中没有这个字段），而 `Save` 是整行写——于是每一次编辑都把该列盖成 Go 的零值时间，行读回来声称自己创建于公元 1 年。没有报错，没有告警，值就没了。这一列按定义不可变，所以现在让写入**根本碰不到它**,而不是小心地把旧值搬过去。（在复现上一条时撞到的。）**同样的写法也存在于 ACME 的 DNS 凭据**——每次改名或轮换密钥都会抹掉创建时间，那是运维在证书续不下来时会去动的页面；一并修了，规则现在住在一条跨仓储的测试里，而不是每个仓储各自重新发现。

- **流量轮询周期改了会生效了，而且能看到当前真正在用的是哪个。**（详见下方「新增」里的诊断页——这两件事是一起被发现的。）

- **周期用量在缺少 snapshot 时不再报 0。** `periodUsage` 早就被改成从用户行 O(1) 算出（它自己的注释写着「lifetime − baseline，不读 DB」），但它的公开包装 `CurrentPeriodUsage` 没跟着改：它仍然去读最新 snapshot，**并且仍然对这个读的结果做分支**。

  `LatestForUser` 在用户没有 snapshot 行时返回 `(nil, ErrNotFound)`，而那个分支把 `ErrNotFound` 和真正的 DB 故障**一起**吞成 `(0, nil)`。于是：一个被调用方压根不使用其结果的读，决定了最终答案；而对一个自己的计数器明明白白写着用了多少的用户，这个答案是 0。

  唯一的生产调用方是 `user.trafficFloor`，所以这是执法漏洞而不是显示错误：`TrafficFloorBytes(limit, 0)` 返回**完整配额**，PSP 于是把「面板已记录用量 + 一整份配额」推下去作为面板侧上限。一个本已用掉大半周期额度的用户，在下一次 PSP 中断期间**又拿到了一整份**。

  修法是不再读。缺失或读不到的 snapshot 对「这个人用了多少」不构成任何陈述——lifetime 和 baseline 都在用户行上，从来没有疑问。顺带去掉了一次每次推送都发生、只为被丢弃的 DB 往返。

- **轮询周期改了会生效了，而且能看到当前真正在用的是哪个。** 流量循环在启动时把周期值抓进变量后就再也不看，所以改 `cron_traffic_pull_minutes` 对它是**静默无效**的——而它正是负责计量、执行配额、采集在线 IP 的那个循环。健康检查循环就在它上面三个函数处每轮重读，注释还写着「与 geo/cert 循环保持一致」；唯独没覆盖到最要紧的这一个。

  这两件事是一起被发现的：诊断页读同一个设置行并且相信了它。把周期从 5 分钟改成 1 分钟后，一个 4 分钟的窗口看起来就是「3 个周期，已稳定」，而此时 `psp_poll_total` 确实还是 0——于是页面对一个**完全健康**的进程报了 critical「流量轮询已死」。判据本身没错，错的是喂给它的那个**尚未生效**的值。

  所以循环现在发布 `psp_poll_interval_ms`：它的 ticker **实际持有**的周期，而不是被请求的那个。页面优先用这个 gauge，只有面对更旧的服务端才回落到设置行。改用「发布被请求的值」只是把同一个谎言下移一层——从管理员保存到循环下一次醒来之间，两者**必然**不一致（睡眠中的 ticker 无法被抢占）。这段延迟是结构性的，现在写在注释里，而不是被糊过去。

- **语言包的条件请求不再被全局 no-store 打掉。** 这是打开运行中的面板看出来的，不是测出来的：200 是对的，304 是错的。语言包路由是 `/api/` 下**唯一**刻意选择「可以缓存」的端点，而它只在返回正文的那条路径上设了 `no-cache`，条件请求那条提前返回了。于是 `/api/` 加上全局 `no-store` 之后，一次成功的重验证被回以 `no-store`。

  这不是外观问题。304 会**更新已存储响应的头**（RFC 9111 §4.3.4），等于告诉浏览器把它刚刚验证过仍然有效的那份条目丢掉：每次加载都退回完整正文的 200，ETag 一分钱都不值。原来的单元测试只覆盖了 200，看不见这个。

### 新增

- **诊断页（报表 → 诊断，仅管理员）。** Phase 0 那批进程内指标第一次有了读的地方，不用再开控制台。

  这个页面只有一条规则，其余全部由它推出：**0 要看窗口，非 0 不看。** 一个 0 在窗口够长之前什么都不能说明，所以窗口不足一个轮询周期时数字被**整体扣住**而不是灰掉——灰掉的 0 仍然是屏幕上的一个 0，而且会被当成 0 读；而一个已经发生的错误就是已经发生了，任何时候都照报不误。

  顶部先回答「每个下游区域到底有没有输入」（五个前置条件），否则「哪儿都是 0」会被读成 N 个独立故障，而不是一个上游原因。样本数不足以支撑分位数时，直接说「样本太少，最大值是 X」，而不是把最大值套上 p95 的名字端出来。

  仅管理员可见，与操作员角色无关：这份快照会暴露部署形态（每周期活跃用户数、面板数、面板有多慢），那是所有者的事。

- **用户编辑页现在会说明「你填的这个连接上限，实际会发生什么」。** 表单**按用户**收一个数，两个上限却**按 (面板, 客户端 email) 各自独立执行**——这两个量不相等，而在此之前界面从不说明。运维方读到的「并发 IP 上限 2」是一个系统并不作出的承诺。

  结论按**这个用户自己的**面板算，规则只有两条且刻意不对称：

  1. **漏洞压倒倍数。** 只要该用户的某一台面板执行不了，他走那台就行——全局上限不是「更高」，是**没有**。所以报「整体不生效」而不是报倍数；显示成「×2」是**错**,不是保守。
  2. **未知不等于正常。** 探测没有答案的面板单独归类，绝不与在执行的面板合并计数——与 `IPLimitEnforcement.Enforcing()` 对 `unknown` / `unsupported` 的处理同一条纪律。

  都不适用时才报倍数：**全局可达 = 输入值 × 该用户的面板侧客户端行数**。这个数字以前不存在于任何界面。

  **顺带修掉一个每天都在发生的误报**：原先的提示按**整个部署**的面板列表判断，所以只要部署里存在一台 S-UI，**每个**用户的编辑框都会警告，哪怕他在那台上根本没有客户端。创建表单保留旧提示——那时用户还不存在，没有客户端可数。

  呈现上只留一个短结论（「实际可达 4」「整体不生效」…）加一个图标，**点开**才是完整原因和面板名：原因必须拿得到，否则读到「整体不生效」却查不出是哪台，只能盲信或无视；但整句铺在字段下会随输入实时改变表单高度，那是警告失效的另一种方式。

### 文档

- **ADR 0024 记下了自研节点后端的决定**，连同证明「性能不是动机」的生产数字。真正的动机是兼容税——尤其是 API 调用层面的变更，影响范围太广。
- **`docs/psp-node-agent.md`**：以我们自己的模型为主，3X-UI 和 S-UI 变成我们向下兼容的对象，而不是我们追着适配的目标；后端独立成仓库，以及这么做带来的那个陷阱。
- **ADR 0025**：推送与拉取到底怎么决定（Q0–Q5 判定规则），以及 PSP 今天为什么没得选。
- **`docs/admin-edit-state.md`** 补上 §1.1：编辑后读回旧值的根因，其实在 HTTP 缓存层。

### 致谢

- **@MLChinoo**（PR #30）：修好了管理端编辑后不刷新的问题，并写下了这块的状态同步文档。顺着这个症状往下挖，才找到根因在 `/api/` 响应缺少缓存指令——现在一并补上了。

## v3.9.2-beta.17 — 2026-09-07

**两个「写入成功、实际什么都没发生」。** 一个让管理员的停用指令可能从未到达面板、而任何一处都不会告诉你；另一个让导入的 Reality 节点把缺少 flow 的链接发给用户，同时 reconcile 全程报绿。

> **升级后在用户页点一次「一键同步 Reconcile」，看「巡检结果 → 未修复问题」。** 出现 `flow_render_divergence` 的节点，说明它渲染出去的订阅链接**一直**缺少服务端要求的 flow。进节点编辑把 Flow 填上即可——报告里写明了该填什么值。**这次之前导入的节点不会自动修复**，原因见下。

### 修复

- **共享客户端的推送失败不再被丢弃。** `pushClientConfigToAll` 丢掉了 `syncSharedLifecycle` 的返回值，然后因为紧接着查询的旧 ownership 列表恒为空而返回 nil ——共享客户端迁移完成后，那张表已被 DROP。

  于是在任何稳态部署上，这个函数**结构上无法报告推送失败**。而它的六个调用方全都以「返回 nil = 已送达」为前提工作：

  | 受影响的路径 | 本该发生 | 实际发生 |
  |---|---|---|
  | 管理员停用账号 / 配额到期自动停用 | 失败则入队 `user_push_config` 重试任务 | 什么都不入队 |
  | 修改到期时间、流量上限、服务状态 | 同上 | 同上 |
  | 应急访问授予、服务暂停、服务恢复 | 同上 | 同上 |
  | `psp_push_config_error_total` 指标 | 计数 +1 | 恒为 0 |

  也就是说：一个你以为已经停掉的用户，可能仍然在节点上正常使用，**没有任何重试在排队、没有任何计数在增长**，只有一行日志。这条路径正是账号停用和超额切断共用的那一条。

  旧注释是这么写的：「不删除旧 fallback 的调用方（每轮刷新、reconcile 自愈）可以忽略返回的错误，反正这里已经记了日志」。这句话在**每个用户都还有旧 fallback 兜底**的时候是对的；迁移把 fallback 拿走了，却把这条许可留了下来。**一行日志不是一次重试。**

- **导入已有 inbound 时采纳它自己的 flow。** `Node.Flow` 此前**只**来自导入表单，而表单不预填。在一个 VLESS + Reality 的 inbound 上把这一栏留空——最常见的情况——就进入了没有任何两个读取方意见一致的状态：

  - 旧的按节点推送路径会**回落到面板自己的 flow**，所以推送收敛、reconcile 每一项检查都报绿；
  - 渲染直接读 `Node.Flow`，于是**递给用户的链接少了服务端要求的 flow**；
  - 共享客户端路径**完全没有回落**（`clientplan` 只看 `Node.Flow`），于是客户端**被创建出来时就没有 flow** ——这正是 `resolveFlow` 注释里记载「已修复」的那个回归，在取代它的那条路径上原样复活。

  导入的语义是接管已经存在的东西，所以现在**空值采纳 inbound 自己的 flow**。只填空：你手动填过的值一个字都不会动，非 VLESS 协议不采纳任何东西，settings 解析失败只损失这个默认值、不影响导入本身。

- **flow 分歧检测器搬到了能真正跑到的地方。** 它此前住在 `checkOne` 里，而那个循环只遍历**旧 ownership 表**——共享客户端迁移会 DROP 掉它。也就是说：在任何已迁移的部署上，为 3X-UI 3.7.0 添加的这个检测器**一次都不会执行**，恰恰是在 3.7.0 让它要检测的状态变得可达的地方。

  现在它在 `checkNodes` 里，按**节点**报一次而不是按客户端——`Node.Flow` 本来就是节点的列，一个空值就是一个问题，不该随节点上的用户数翻倍。

  **仍然只报告、不自动修复。** PSP 该渲染什么 flow 是运维的决定，不是 reconcile 该悄悄扫过整个 fleet 去改的漂移。这也是本次之前导入的节点不会自愈的原因：新导入的不会再出问题，旧的会被报出来等你处理。

### 文档

- **配额安全网的误差预算补上了最大的一项。** `PanelQuotaCap` 的注释逐条列举了它可能偏松的两个原因（读数滞后一个轮询周期、面板计数器被重置），两个都是瞬态自愈的，却漏掉了第三个——它更大，而且**不自愈**：

  `SyncUserLifecycle` 把**同一个全局 headroom** 发给用户的每一个面板，各自重基到该面板自己的计数器。于是横跨 `P` 个面板的用户，**每个面板都独立放行 `headroom` 字节，合计 `P × headroom`**。

  这一项修不掉。被执行的谓词是 `Σ_面板 用量 ≥ 配额`：PSP 拥有配额，每个面板只拥有其中一个加数、且看不见其他加数——不存在任何推给单个面板的值，能让它执行一个它看不见的和。正常运行时不产生任何影响（PSP 自己那层执法用的是求和后的用量，精确），放大只在 PSP 无法轮询期间生效——而那正是这个 cap 唯一覆盖的窗口。**纯文档，行为未变**，算术由测试钉住。

- **ADR 0025：推还是拉的决策规则。** 「配置该由节点拉还是 PSP 推」这个问题被反复重新提出，本 ADR 记录判据本身（状态还是调用 / 谁拥有真相，含 JOINT 与 CONTESTED / 是否控制节点上的软件 / 截止时间在读还是在写、能否委派 / 分开存 + 够新 + 覆盖度 + 否定的出处 / 并发），以及一个结论：**PSP 今天在这个问题上没有选择权**——一个「节点」是第三方 3X-UI/S-UI 里的一行配置，PSP 不在上面放任何代码，让节点主动拨号是向上游提的功能请求而不是设计选择。该讨论随 ADR 0024 一起解冻，不早于它。

  顺带修正了 `reconcile` 的包注释：它描述了三个触发点而实际只有一个存在（L1 自称未接线，L2 以现在时写着但同样不存在），并且声称「所有检查只作用于 ownership 表的行」——这对节点轴从来就不成立。

## v3.9.2-beta.16 — 2026-09-02

**并发 IP 上限从此不会再悄悄失效。** PSP 现在主动探测每台节点是否真的能执行它，并把结论显示出来。仍然是纯观测：不改推送、不改能力声明、不动任何用户。

> **升级后先看一眼服务器列表。** 节点状态下方出现红色「IP 上限不生效」徽标的，说明你在那台机器上填的并发 IP 上限**一直没有起过作用**。悬停徽标会写明是哪一道闸、怎么改。

### 新增

- **节点侧执行前提探测** —— 一台会封禁的节点和一台什么都不做的节点，从 PSP 这边看**完全一样**：`limitIp` 写进去成功、读回来一致、界面显示正常。生效与否取决于机器上的一个二进制（fail2ban）和一个环境变量（`XUI_ENABLE_FAIL2BAN`），PSP 已有的任何一次读取都看不见它们。3X-UI 3.7.0 为它自己的界面加了 `GET /server/fail2banStatus`，正是为了同一个原因；PSP 现在也问。

  探测**跟着已有的版本探测一起走**（挂在流量轮询上，周期 `cron_traffic_pull_minutes`，默认 5 分钟）——同一块面板、同一个已登录会话、多发一个 GET。管理员点「测试连接」也会立刻重探一次，刚装完 fail2ban 不必等下一个周期。

  | 状态 | 含义 |
  |---|---|
  | `enforced` | 断连 + 防火墙封 IP，完整生效 |
  | `disconnect_only` | Windows：会断连，但不封 IP，对方可以立刻重连 |
  | `disabled` | `XUI_ENABLE_FAIL2BAN` 被设成了 `true` 以外的值 |
  | `not_installed` | 闸开着，但节点上没装 fail2ban |
  | `unknown` | 本该能回答的面板没给出答复 |
  | `unsupported` | 面板低于 3.7.0，没有这条路由 |

  **`XUI_ENABLE_FAIL2BAN=1` 是这次探测的主要目标**：它看着像「启用」，实际把整条执行路径关掉（上游只认字面量 `"true"`）。这个错误最有可能正躺在某台真实节点上，而在此之前它在面板和 PSP 里都完全看不出来。

- **呈现在两处**：服务器列表里节点状态下方的徽标；以及**用户表单里填并发 IP 上限的地方**——会直接点名哪些面板「存得下但不会执行」。新指标 `psp_ip_limit_enforcement_total{state}`。

### 三条刻意的判定规则

- **`!enabled` 排在 `windows` 前面。** 上游 `Run()` 在看平台之前就检查环境变量，所以一台设了 `XUI_ENABLE_FAIL2BAN=1` 的 Windows 节点同样什么都不执行。先看平台会把它报成正常。

- **`usable` 直接采信上游，不在 PSP 侧重算成 `enabled && installed`。** 今天两者等价，但那是上游的复合判断：以后它多加一道闸，采信会让 PSP 跟着降级，重算则会永远宣称「生效」。

- **探测失败时什么都不写，保留上一次的结论。** 一次网络抖动把 `enforced` 覆盖成 `unknown`，会把运维引向一个不存在的故障。时间戳记的是**这个结论是什么时候得出的**，不是最后一次尝试的时间，所以陈旧本身是看得见的。

`unknown` 与 `unsupported` 刻意分开，两者都既不算「生效」也不算「需要处理」：前者意味着**探测器自己坏了**，值得去查；后者意味着这个面板版本没法回答，无从下手。合并会让每台旧面板挂上一个永久且无解的告警——而运维学会忽略的告警，会连带着让别处真正要紧的那个也被忽略。同理，`unsupported` 不画徽标（版本一栏就在旁边），`unknown` 画灰色（坏掉的探测器绝不能长得像干净机群）。

**这不是「能力」。** 能力回答的是「面板存不存得下」，而每个受支持的 3X-UI 都存得下。把两者合并，要么会让一块存得下的面板停止漂移愈合、每周期重推重启内核，要么会宣称一份看不见的执行力。详见 [`docs/connection-limits.md`](docs/connection-limits.md) §5.2。

## v3.9.2-beta.15 — 2026-09-02

新增**异地并发检测**：把一个用户在所有面板上的并发源 IP 合起来看，同时出现在多个国家就是共享账号的信号。**只观测和告警，不做任何自动处置**——嫌疑不是证据。

同时更正一条 beta.14 说错的事。

### 新增

- **按用户的并发 IP 聚合** —— 管理员填的上限意在约束**一个人**，而下面每一个执行点约束的都是别的东西：3X-UI 的 `limitIp` 按 client **email** 计，PSP 又会把一个用户在同一面板上按凭据分区拆成多个 email、再乘以面板数。每块面板只看得见自己那份，所以**「这个人现在用了几个地址」这个数在系统里任何地方都不存在**。现在算出来了，在唯一持有 `(面板, email) → 用户` 映射的那一层。

  **覆盖率是带着走的**：读不到的面板让相关用户的数字变成**下限**而不是总数，界面上带警告标记。把下限当总数显示，会在证据缺失时读作「这个账号没问题」。

- **异地并发判定，做成策略而不是规则** —— 「同时在两个国家就是共享」这条朴素规则两个方向都会错，而且会**抖动**：一次噪声报警、下一次解除，运维方很快学会忽略它。每个旋钮都对应一类会被误判的真实用户：

  | 旋钮 | 默认 | 它保护谁 |
  |---|---|---|
  | 判定粒度 | 国家 | **异城市不算**——通勤、运营商 NAT 池、手机切基站每天都在发生 |
  | 容错率 | 1 | 第二个地点常常正常（公司 VPN、第二住所、境外家人） |
  | 连续几次才标记 | 3 | 一次噪声只到「疑似」，不报警 |
  | 连续几次才解除 | 6 | 刻意比上升慢，否则踩一下线下就能甩掉标记 |
  | 允许任何地方 | 关 | 出差账号、团队共享凭据的**整体豁免** |
  | 视为同一地点的组合 | 空 | 真实跨境常客（如 `JP,TW`）；组外的第三个地点仍然触发 |
  | 最低可定位比例 | 0.5 | 库陈旧就判「无法判断」，不悄悄变成「全员清白」 |

  全部**按分组可覆盖**，继承模型和流量/连接额度一致（用户 > 分组 > 全局）。非法值一律向「不指控」修复——容错率 0 会让每个在线用户（包括坐在家里的）都被标记。

- **七种状态，不是布尔**：`已标记` / `疑似` / `正常` / `无法判断` / `无连接` / `已豁免` / `未启用`。**只有「已标记」代表持续超限**；「疑似」是尚未达次数的爬升过程。其余四种意味着检测器不在能判断的位置上，而它们看起来都像「零个标记」——所以 `psp_geo_verdict_total{state=...}` 按状态分标签，界面上「无法判断」也**绝不使用绿色**：geo 库悄悄坏掉的机队，如果和「正常」同色，看起来就和干净的机队一模一样。

- **界面**：设置页新增「异地并发检测」区块（七个旋钮，提示写的是「它保护谁」）；日志页新增「异地并发」标签，每行带**判定依据**而不只是状态——「已标记」而没有「因为连续 3 次同时在 2 个地方，容错是 1」不构成动一个账号的理由。

- 新指标：`psp_user_live_ips`（每用户并发地址数分布）、`psp_geo_verdict_total{state}`、`psp_live_ip_users_incomplete_total`。
...
> **如果你有 3X-UI 低于 3.7.0 的面板、且给用户设过设备数上限，建议尽快升级**——那种组合下面板节点在每个轮询周期都会被重启一次 Xray 内核。判断方法：诊断端点里看 `psp_lifecycle_sync_write_reason_total` 有没有持续增长的 `reason=device_limit`。

### 修复

- **【严重】设备上限在 3.7.0 以下的面板上造成永久写循环** —— `limitHwid` 是 3X-UI 3.7.0 才有的字段，而 PSP 支持到 3.4.2。老面板接受这个键但不保存，读回永远是 0，于是每个周期都判定「我要 N、面板是 0、必须写」，发出一次**全量客户端替换 + Xray 内核重启**，永不收敛。

  本来有防线：`lifecycleWriteReason` 会跳过面板存不下的字段，正因为那种比较永远无法收敛。它靠能力列表判断，而 3X-UI 适配器**无条件声明**了这个能力（注释理由是「能力按适配器静态定义，看不见面前的面板」）。**两处注释互相矛盾**，防线从未生效。同一个静态能力也让 `reportCapabilityGaps` 保持沉默，所以连计数都看不到。

  代价是 beta.9 对这些用户完全作废：那一版把写入从 473.3/小时降到 2.3，而这个缺陷给每个受影响客户端每周期重新加回一次写入，每次都带一个内核重启。

  能力现在跟随面板版本。版本不是猜的——`servers.panel_version` 每 10 分钟探测一次，且探测失败时**刻意保留上次已知值**。版本未知或无法解析时判定为「不支持」，这个方向是刻意选的：两种错误不对称，误报支持会产生上述重启循环，而误报不支持最多让这个字段不再单独触发写入（值仍随其他字段的写入一起下发）。只有从未连通过一次的面板会读作未知。**只有 `limitHwid` 受版本限制**，IP 上限在所有受支持版本上都存在，对它设限反而会让未探测面板上的上限静默失效——有测试钉住这个不对称。

- **负数流量配额被存下并读成「不限」** —— 两个连接上限一直有校验，流量没有。而负值不是「很小的配额」，是**没有配额**：`trafficFloor`、`PanelQuotaCap`、超额判定**全部**用 `> 0` 测试，所以存进去的 `-1` 在每一处都读作不限——管理员想收紧，实际放开了。创建与更新两条路径各自补上校验。

### 新增

- **`psp_user_clients_per_panel`** —— 用来拆开 `psp_user_client_count`（成本模型里的 P）里揉在一起的两个乘数。

  连接上限是按面板侧客户端 email 分配预算的，而 `clientplan.Build` 是**每用户每面板**调用的，所以 P 同时包含「用户在几台面板上」（面板互相看不见，多面板部署的固有性质）和「同一面板内裂成几个客户端」（PSP 按 `(password 类, flow)` 拆分，因为一个 3X-UI 客户端行只有一个 password 槽和一个 flow 槽）。**只有后者是 PSP 把管理员填过一次的上限乘开了**，而 P 分不出这两者：4 台面板各 1 个客户端、和 1 台面板裂成 4 份，P 都是 4。

  新指标只测第二个因子。**p50 与 p95 都接近 1** = 分裂没有发生，倍数来自跨面板、属固有性质；**明显大于 1** = 分裂在发生，且倍数就是这个值。界面文案要等这个读数——归错原因的提示比没有提示更糟，它会让人去调一个不是原因的东西。

## v3.9.2-beta.13 — 2026-09-01

对 SSO 权限路径做专门的安全审查。查出两条，第一条比分组分流这个功能本身老得多。

### 修复

- **【严重】SSO 的角色降级绕过「最后一个管理员」保护** —— 管理 API 在**三条**路径上都拦着这件事（降级、删除、停用），理由就写在代码里：否则面板会没有人能管理。而 `reconcileSSOUser` 写的是同一个 `role` 列，却一条都没经过。

  实测：唯一一个启用的管理员，IdP 那边把他移出管理员组，一次登录之后 `CountEnabledAdmins` **归零**。全新安装总有一个带本地密码的 bootstrap 管理员兜底，但把它删掉、或者全部管理员都只靠 SSO 的部署，就是**一次目录侧编辑换来一个无人可管的面板**。

  同一条件现在也在 SSO 路径上执行，但**拒绝的方式不同**：管理 API 返回错误即可，这里不能——让登录失败等于把人整个挡在外面，比一个过期的角色更糟。所以是**拒绝这次降级、打警告、登录照常继续**，其余角色变更不受影响。管理员计数读取失败时**向安全侧失败**（当作「可能是最后一个」而拒绝）。

  这条**不是本轮引入的**，在分组分流之前就存在。

- **沉默守卫是一个撤销绕过，而且原本完全不可见** —— beta.12 加的守卫刻意 fail-open（IdP 没发声明时保留已存的角色与分组），挡住了「一次目录侧意外把整个机队降级」。代价必须说清楚：**能让声明消失的人，就能保住目录已经收回的授权**，而 Entra 的 group overage 由用户自己所在的组数触发。

  取舍保留，但不再隐形：新增 `psp_sso_claim_silent_total{kind=role|group}`，在「配了规则、但 IdP 一个相关属性都没发」时计数。**长期高于零**意味着 IdP 停发了 PSP 依赖的声明，或者有人正握着目录已经收回的授权。

## v3.9.2-beta.12 — 2026-09-01

对 beta.11 的分组分流做对抗性审查，查出**降级分支本身**的两个缺陷。两个都会把「撤销权限」变成别的东西，建议从 beta.11 尽快升级。

> **谁受影响**：只有**已经配置了分组规则**的部署。没配规则的（升级后的默认状态）不受影响，因为规则集什么都不主张时不会有任何降级发生。

### 修复

- **【严重】没配默认分组时，降级会发放无限额度** —— 降级分支无条件回落到 `default_group_slug`。该值为空（全新安装的状态）时解析成 `GroupID=0`，而**不属于任何分组的用户什么都继承不到**，`ResolveLimits` 给出 0/0/0 —— **流量不限、连接上限不限**。于是被 IdP 撤销的人拿到的比撤销前**更多**，正是这条分支意义的反面。

  后续重推还会静默成功：`ResyncMembership` 对 `GetByID(ctx, 0)` 拿到 `ErrNotFound`，而 `runUserTask` 把 `ErrNotFound` 读作「用户已删除」并把任务标为**成功**，旧 OU 的入站因此永远不会被摘掉。

  两端都修：**保存时拒绝**——有分组规则却没有默认分组的配置存不进去（能降级的规则集必须有降级的去处）；**运行时兜底**——没地方去就不降级，保持原样。

- **【严重】IdP 没发声明被当成「明确撤销」** —— 匹配函数分不清「IdP 说你不在那个组」和「IdP 什么都没说」，而降级对两者一视同仁。第二种和用户本人无关：**Entra 在用户加入约 150 个组以上时停发 `groups`、改发 `hasgroups`**；声明映射可以被改坏；新建的应用注册可能根本没配这个声明。

  任何一种都会变成**机队级权限变更**——每个 SSO 用户在下次登录时离开自己的 OU、失去它的配额与节点集——起因是一次目录侧编辑，而做那次编辑的人根本没把它和 PSP 联系起来。

  现在把「沉默」与「否认」分开：只有规则集引用的属性**真的出现在断言里**才可能降级。**声明存在但不含该组仍然照常降级**，否则这个修复等于把功能关掉。

  **角色规则有完全相同的形状，同一轮一并修掉。** 起初只改分组侧，理由是角色降级掉到 `RoleUser`、代价小得多；那个理由只站得住一半——缺陷同源、修法现成，而 Entra 一次 group overage 就会让**所有靠规则拿到 admin / operator 的人在下次登录被降成普通用户**，面板可能因为一次目录侧编辑失去全部特权用户。判断「IdP 是否发过声」现在只有一份实现，两种规则共用，并有测试钉住二者一致（理解不一致会在同一个断言上朝相反方向出错，且静默）。

  **这是一处行为变更**：IdP 不下发规则所引用的属性时，角色不再被降级。方向是更安全的一侧——沉默不再等于撤销——但如果你的部署依赖「IdP 停发声明即撤销管理员」这一行为，请注意。

- **没有规则的部署每次 SSO 登录白读一次分组行** —— 答案不可能改变结果，而这次读取失败还会为一个没人配过的功能打出 `sso group rule unresolved` 警告。

- **校验器把任何分组查询错误都报成「没有这个 slug」** —— 一次连接抖动会让运维方去一个正确的值里找拼写错误。现在只有确实查不到才这么说。

- **分组列表加载失败时，规则编辑器把每条已存规则标成「该分组已不存在」并禁用添加** —— 空列表和请求失败长得一样，把前者当后者，就是把一次网络抖动呈现成配置损坏。现在记录加载结果而不是吞掉。

## v3.9.2-beta.11 — 2026-09-01

承接 beta.10：既然 OU 承载了额度，「谁属于哪个 OU」就不能再只有一个静态答案。SSO 现在按 IdP 属性把用户分流到对应分组，**且每次登录都重新判定**。

> **行为变更，请先读这一段。** 配置了分组规则之后，**已有用户**的分组会跟随 IdP 变动——被移出某个 IdP 组的人会离开对应 OU、落回默认组，连带失去该 OU 的配额与节点。**升级本身不会搬动任何人**：没配规则时规则集什么都不主张，所有存量部署升级后即为此状态。搬动只在你写下第一条规则之后开始。

### 新增

- **按 IdP 属性决定用户进入哪个分组（OU）** —— 详见 [`docs/connection-limits.md`](docs/connection-limits.md) §11。此前 SSO 建号只有一个去处：`default_group_slug`，一个静态的桶。IdP 送来的组和属性早就解析好放在 `EnsureSSOInput` 里，却只喂给角色匹配器，**从没参与过 OU**。

  现在有一组 `group_rules`：有序、首条命中者胜、精确比对，命不中则落默认分组。形状与既有的角色规则一致，**匹配共用同一份代码**——两种规则声称「匹配方式一样」却各持一份实现一定会漂移，而第一次分歧是静默的（一条规则对角色生效、对分组不生效）。

  **刻意做成独立的一组规则**，而不是在角色规则上加个字段：「谁是管理员」和「谁属于哪个 OU」很少是同一个条件，塞进同一个 first-match 列表会强迫两个决定共享一套优先级。

  **每次登录都重新判定，不是只在建号时。** 初版是「只在建号时定」，被指出这留了口子后改掉：角色本来就是每次登录同步的，同一套 IdP 信号在两件事上两套规则没有依据；更要紧的是**不同步的错误方向是「权限过大」**——IdP 把某人移出支撑高级 OU 的那个组而 PSP 永不复查，那份高配额和高级节点就是永久的。

  而「升级不该悄悄搬动线上用户」这条顾虑**不需要开关**，它由角色引擎现成的 carve-out 保证：没有规则主张任何 OU 时谁都不动；无人主张的 OU 视为面板自管（管理员手工放的，IdP 不该管）；`Keep=true` 让手工授予的 OU 在规则未命中时保留。

  **两条方向相反的失败策略。** 规则指向不存在的分组时：**建号时拒绝建号**（静默回落可能把本该被限制的人放进更宽松的默认组，且此刻还没有服务可打断）；**登录时记日志、保持原样、不阻断登录**（存量用户因为规则目标被删就登不进来，比一个陈旧 OU 糟糕得多）。真正拦住这件事的是**保存时校验**，会点名出错的行号与 slug——否则运维方的拼写错误要由别人的登录失败来告诉他。

  **改了 OU 必须推到面板**：存储行本身改不动节点上任何东西，变更走与管理员手工换组同一条重推路径。反过来也成立——**没有发生迁移的登录不推送任何东西**，否则每次 SSO 登录都会把 `UpdateClient` 与 Xray 重启扇出到整个机队。

  **界面**：设置 → SSO，SAML 与 OIDC 各一份，紧挨角色规则、同样可拖拽排序。目标是**真实分组的下拉选择器而非自由文本**（角色可以合法地填一个后端还不认识的字符串，分组不行）；指向已删除分组的规则保持可见并标红，不会在打开表单时被悄悄改写。

  **实机验证**：真实面板 + 真实浏览器。三条拒绝路径各返回 400 并点名行号与 slug，被拒绝的保存不覆盖已存规则，SAML 与 OIDC 同一个校验；浏览器里已存规则渲染为分组名称（存的是 slug），下拉是真实目录，改选后保存回读确认落库且 `keep` 未丢，无 console 报错。解析矩阵逐分支变异验证，另有测试断言 `EnsureSSO` 确实调用了它、面板确实被通知。

### 修复

- **三处「0=不限」的文案在 beta.10 之后就不再成立** —— 自助注册、SAML 新用户、OIDC 新用户的默认流量输入框都写着 `0` 表示「不限」。三态落地后 `0` 表示「没有意见，跟随分组」，而分组可能有配额，所以填 `0` 恰恰**可能不是**不限。这正是第三态要消灭的歧义，而唯一告诉运维方该填什么的那行字一直没改。现统一为「0=跟随分组」。

- **浏览器挂死被工具报成 React 渲染失败**（beta.10 发布时实际发生过）—— `smoke:dist` 报 `production browser smoke failed: React root rendered, login form rendered, MUI SVG icon rendered`，读起来就是打包产物什么都渲染不出来。它渲染得好好的：Chrome 挂死后**优雅处理 SIGTERM 并返回 0**，于是超时 kill 在退出码上完全不可见，空 dump 被兜底成「root 是空的」，三条内容检查理直气壮地失败。另有 `lastError` 被后续候选覆盖，导致真实原因变成「找不到浏览器」。现按失败种类区分，只有 NOT_FOUND 才继续试下一个候选，并区分「一个字节都没拿到」（运行环境故障，检查根本没跑）与「拿到内容但结构不对」。用 stub 浏览器实跑八种情形验证，其中「挂住后 SIGTERM 退出 0」在旧脚本下逐字节复现了线上那条报错。

- **建号路径「继承」的测试缺口** —— `LimitOverridesFromCreate` 是每条建号路径把标量 `0` 读成「没有意见」的唯一依据，此前一个测试都没有，也没有任何测试断言那些路径确实调用了它。行为本身是对的，但坏起来完全静默：映射反了则每个新建用户都被钉出分组策略。已补 domain 与服务两层覆盖，四处变异验证。
