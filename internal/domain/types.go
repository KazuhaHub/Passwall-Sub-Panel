...
type RuleSet struct {
	...
	// supports and ignores Mihomo-only control-flow rules.
	Content string
...
// selector. Kind is one of: builtin, proxy_group, node, node_set, rematch.
// NodeID is used only by node; Value carries the builtin/group/rematch name
