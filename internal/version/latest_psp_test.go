...
func TestPSPBehindStable(t *testing.T) {
	cases := []struct {
		...
		// AND THE SAME READ UNDER THE CURRENT NAMESPACE, which is what every
		// release from now on arrives as.
		{"4.0.1.2", "v4.0.2", true},
		{"4.0.1.2", "v4.0.1.2", false},
		{"4.1.0", "v4.0.1.2", false},
		// AN OLD STABLE RELEASE IS READABLE NOW. `v3.7.0` is a tag under the
		// current namespace and names a version the product line has long passed,
		// so the comparison is what keeps the nudge silent — not the tag rule.
		{"4.0.0", "v3.7.0", false},
		{"3.7.0", "v3.7.1", true},
...
// A LATEST THAT IS NOT ONE OF OUR TAGS IS NOT A LATEST. Shapes reach here from
// outside that are not addresses this project publishes: a bare version
// (`4.0.0` — a version, not a path), a namespace with no version after it, and
// the legacy v-prefixed beta, which begins with the current namespace and fails
// inside it. Accepting any of them would build a nudge around a release the panel
// cannot then describe.
func TestPSPBehindStableRefusesALatestThatIsNotAReleaseTag(t *testing.T) {
	for _, latest := range []string{
		"4.0.0", "release/4.0", "release/latest", "latest", "release/", "v", "v4.0", "v0.0.1-beta11",
	} {
...
func TestAcceptLatestPSPStable(t *testing.T) {
	cases := []struct {
		...
		{"v4.0.2", false, "v4.0.2", true},
		{"v4.0.0", false, "v4.0.0", true},
		// THE FOUR PUBLISHED BEFORE THE ADDRESS CHANGED ARE STILL RELEASES. They
		// cannot be renamed, and a nudge that stopped reading them would go silent
		// over an installation that is genuinely behind.
		{"release/4.0.0", false, "release/4.0.0", true},
		{"release/102.1.0", false, "release/102.1.0", true},
		{"v4.0.0", true, "", false}, // GitHub flagged it prerelease
		...
		{"4.0.0", false, "", false},
		{"4.0.0", true, "", false},
		// AN OLD STABLE RELEASE IS A RELEASE. `v3.7.0` is what the newest
		// non-prerelease on this repository may be while every product release is a
		// testing candidate, and reading it as a tag is exactly what the version
		// comparison downstream is for.
		{"v3.7.0", false, "v3.7.0", true},
		// THE LEGACY BETA IS NOT ONE OF OURS, flag or no flag. It begins with the
		// current namespace and fails inside it: a hyphen is not a digit, and its
		// release line was never published under a product scheme.
		{"v0.0.1-beta11", false, "", false},
		{"v3.7.0-beta.16", false, "", false},
		{"v3.7.0-beta.16", true, "", false},
		// SHORTHAND AND NONSENSE.
		{"v4.0", false, "", false},
		{"v4.0.0.0", false, "", false},
...
// tag is integers behind a namespace, with no hyphen anywhere, so the test found
// nothing to reject there and the code carried an exemption to keep it from being
// applied where it was meaningless. With that shape gone there is no gap and no
// exemption: the flag is the whole decision, and the tag only has to be one of
// ours.
func TestStableSelectionIsTheFlagAndNothingElse(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "a testing candidate", tag: "v4.0.0", prerelease: true, want: false},
		{name: "a released candidate", tag: "v4.0.0", prerelease: false, want: true},
		{name: "a candidate the flag calls released", tag: "v4.0.1", prerelease: false, want: true},
		{name: "a historical address", tag: "release/4.0.0", prerelease: false, want: true},
