...
func TestFetchAndApplyRefusesAnOlderManifest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprintf(w, `{"schema_version":2,"major":4,"updated_at":%q,"entries":[{"psp_min":"v4.0.0-beta.1","psp_max":"v4.0.0-beta.99","max_tested_xui":%q}]}`, updatedAt, maxTested)
	}))
	defer server.Close()
	...
	if got := currentAppliedRevision(); got != "2026-09-19" {
