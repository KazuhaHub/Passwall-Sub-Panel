...
**一次真实发布。** 发布路径已被执行到推广之前（SOP §6 记录了范围与结果：六目标构建＋来源校验、原生运行证据、在真实候选提交上组装出 11 case 无缺的证据索引），但**打 tag、上传归档、推 `:latest`／`:beta` 从未发生过**。R10 的验收项是"发布摘要＝验收摘要、无旁路"这一性质，机制与守卫测试都在；缺的是把这个性质应用于一次真实推广的场合。
