...
HTTP/3 常见的 `UDP/443` 与其他 UDP 分开控制：`🎮 UDP控制` 默认 `DIRECT`（允许 UDP、走本地出口，出口 IP 可能与 TCP 代理出口不同），`⚡ QUIC控制` 默认委托给 `🎮 UDP控制`；两组都可在客户端单独选择节点、`DIRECT` 或 `REJECT`。规则匹配先 QUIC 后普通 UDP，展示顺序则是 UDP 在前。规则不包含国家/地区假设，因此海外用户和回国代理使用同一套配置。详见 [ADR 0030](adr/0030-separate-quic-and-udp-controls.md)。
