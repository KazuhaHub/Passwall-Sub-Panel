...
主要表通过 GORM AutoMigrate 自动创建。规则集/模板当前由 YAML 仓储读取，不以 MySQL 表为真相源。
...
CREATE TABLE users (
  ...
  enabled_rule_sets    JSON,                            -- 历史字段；当前公共规则由模板 rule_sets 决定
  personal_rules       TEXT,
  expire_at            DATETIME,
...
后端:
  ...
  7. 写 xui_clients 表（只记录面板纳管的 client）
...
后端:
  ...
  3. 派生新的各协议密码
  4. 对每个 xui_clients 条目 → SyncSvc.UpdateClient:
       新 uuid / 新 password
...
后端:
  ...
  3. 校验：所有 client 都在 xui_clients 表内？
     - 是 → 继续
     - 否 → 任务失败并记录未纳管 client，UI 在任务详情提示"请先处理未纳管 client"
  4. 对每个纳管 client → SyncSvc.DelOwnedClient + 删 xui_clients 条目
...
后端:
  ...
  3. 写 xui_clients 表，匹配键为 (panel, inbound_id, client_email)
...
SAML 配置通过管理后台 `/api/admin/settings/saml` 读写，落到 `saml_config` 单行表；OIDC 同理落到 `oidc_config`。`config.yaml` 只保留启动所需的监听、数据库、JWT 等最小配置。

| 配置 | 存储 | 说明 |
|---|---|---|
| SP entity_id / ACS URL / 证书私钥 | `saml_config` | 用于生成 SP metadata、签名和验签 |
| IdP metadata URL / 刷新间隔 | `saml_config` | 自动拉取并缓存 metadata |
| attribute_mapping | `saml_config` JSON | UPN、email、display_name、groups claim 映射 |
| admin_group_ids / default_group_slug | `saml_config` JSON / 字段 | 首次登录入组与管理员识别 |
| new_user_defaults | `saml_config` JSON | 首次 SSO 用户默认流量、到期、重置周期 |
...
| 8 | 配置存储 | MySQL（运行时配置）+ YAML（规则集/模板/启动配置） | ✅ 已实现 |
