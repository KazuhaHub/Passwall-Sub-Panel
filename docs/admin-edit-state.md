...
规则集是一个明确的反例：保存时会删除主规则和子规则中已不存在策略组的 `proxy_group_order`、`proxy_group_members` 与 `proxy_group_options` 项，并规范化代理组类型参数；子规则、Rematch 出站及其成员引用也会在落盘前校验。短期旧格式的 `mihomo_rules` 会先合并进主规则。因此 `PUT /api/admin/rules/:slug` 返回实际落盘的完整实体，前端必须用该响应替换本地行，不能把提交前的表单快照写回。
