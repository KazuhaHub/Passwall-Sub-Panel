...
新设置选择未来新任务的策略，不追溯缩短或声称延长已承诺的旧窗口。清理器使用任务自身的
保护截止时间，不能读取当前设置来重新计算旧任务。缺少快照或 deadline 的 legacy 证据保持
保留，不猜测可删除时间。
当前设置更新只写 settings，不改已有 task/quarantine，也未启用清理；身份 tombstone 暂不自动
物理删除。策略设置完成不能替代下面的上线验收。

**PSP 不可变快照已实现**：`NodeAgentTask.Lifecycle` 为可空、严格校验的持久快照，包含
`issued_at_ms`、`not_after_ms`、原始三项策略以及 `full_result_retain_until_ms`。保护下限固定为
原始最晚开始时间加原始完整结果保留天数；构造与读写拒绝非法策略、deadline 不晚于签发时间、
算术溢出及下限漂移。它是保守的最短保护期限，不是过了即可删除的许可证；晚完成结果的实际
保护期限后续只能取更晚值（如完成时间加原始保留期），不能重写该快照。

nullable TEXT 没有默认值：升级不会给 legacy 任务猜 deadline。非 NULL 的坏 JSON、未知字段或
不合法快照在仓储读取时失败，不降级成 legacy。返回值均复制快照，调用方不能改变已存值。
