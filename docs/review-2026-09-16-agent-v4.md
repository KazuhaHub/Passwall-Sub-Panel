...
> **给接手的人：先读「怎么用这份清单」。** 这里面有 **6 条是逐行核实过的**，
> 其余是**线索，不是结论**——把线索当结论处理会浪费你的时间。
>
> **反过来同样要小心：本文档第一版曾把其中一条判为「已推翻」,那个推翻是错的。**
> 它现在是 H-6，一条确认的 HIGH。复核的对象包含判定本身，不只是原始发现。
...
**曾经有过第三格「已推翻」,现在没有了。** 唯一放进去的那条（H-6）经复核是真的，
我的推翻才是错的。见第二节——那一节现在是这份文档里最该读的部分。
...
所以分界线不在「引用可不可信」,而在**结论**——**两个方向都是**:

- 一条**发现**可以引用准确而后果不成立；
- 一条**推翻**同样可以。H-6 就是后者：我读到 `if user.TrafficLimitBytes > 0`
  就下了结论，没读它下面那一行 `append` 在 if 外面。

**复核时要验的是「这个失败场景真的会发生吗」,不是「这行代码在不在」。
而且复核对象包含判定本身，不只是原始发现。**
...
## 二、一条被我错误推翻、现已确认的 HIGH

### 🔴 H-6 组限额缓存冷启动失败，会清掉节点上已生效的配额闸门 ✅
`internal/adapters/sqlstore/group_limits_cache.go:76`

**这条最初就标着 HIGH。本文档的第一版把它判为「已推翻」,那个判断是错的，
决定性的那一行我没读。** 完整链条现已逐行确认：

1. 管理员编辑任意分组 → `invalidate()` 把 `byGroup` 置 nil（冷缓存）
2. 紧接着的一次读遇到瞬时 DB 错误 → 走 `line 76` 返回零值 `GroupLimits{}`
   → 继承分组限额的用户解析出 `TrafficLimitBytes = 0`
3. `nodesync.go:775-795` —— **`body.Quota = append(...)` 在 `if user.TrafficLimitBytes > 0`
   的外面**。所以条目**照发**,只是 `HeadroomBytes` 保持 `nil`
4. 节点 `ApplyQuota`（`NODE: internal/state/sqlite/clients.go:252-267`）:
   `HeadroomBytes == nil` → `baseline` 也留空 → `deriveGate` 返回 `GateUnconfigured`
   → UPDATE 把 `baseline_bytes`、`headroom_bytes` 写成 **NULL**,闸态写成 **unconfigured**
5. **一个本来关闭的闸门被清空并放行。**

**我当初错在哪**：我读到 `if user.TrafficLimitBytes > 0`,就认定零限额意味着
「不发配额条目」,于是搬出「条目缺席 = 保持上次已知」这条协议规则来推翻它。
**但 append 在 if 外面，条目从来就没有缺席过**,那条协议规则根本不适用。
我核对了引用，没核对下一行。

> **这份文档第一版用这条当例子，教人「引用准确不等于结论正确」。
> 结果那个错误结论是我自己的。** 教训不变，例子换了个方向——
> 现在它示范的是：**做推翻的人，和做发现的人一样需要被复核。**

**修复方向**：冷缓存读失败不该产出一个看起来合法的零值。要么向上传播错误、
让调用方跳过这一轮（保持上次已知，与协议语义一致），要么让
`GroupLimits` 能表达「未知」而不是只能表达「零」——
和 `HeadroomBytes` 是 `*int64` 同一个理由。
...
4. **H-6 组限额缓存** —— 触发面很广（任何一次分组编辑都会置冷缓存），后果是**闸门被放开**而不是被误关。
   与 H-1 是同一类：配额的两个方向各错一次，一个该放行的没放，一个该拦的没拦。
5. **H-5 升级签名** —— 爆炸半径最大，但需要设计（签名 or PSP 下发预期摘要），不是一行。
6. **H-4 / reconcile 那条** —— 两条都是「不知道被当成确定答案」,建议一起改，因为它们共用同一个 `ErrNotFound` 语义。
7. **单一真相源侵蚀** —— 不紧急但会持续变差；每多一份拷贝，将来改规则时漏改的概率就翻一倍。
