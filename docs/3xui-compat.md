...
### 2026-09-16 / V3 稳定维护线认证 3X-UI 3.8.5 与 S-UI 1.6.3

V3 从 `v3.9.2` 分叉到长期维护分支 `release/v3`。V3 只接受兼容性和稳定性修复，数据库模型保持不变，不包含 V4 的原生节点后端或迁移。

3X-UI 3.8.5 的面板 API 与 PSP 既有适配器兼容，但其 Xray 26.9.9 要求 REALITY ClientHello 以 `X25519MLKEM768` key share 开头。V3.9.3 回移了已验证的订阅处理：Mihomo 根据面板缓存的 Xray 版本，仅对 26.9.8+ 的 REALITY 节点输出 `client-fingerprint: chrome` 与 `reality-opts.support-x25519mlkem768: true`；当前 sing-box 没有等价开关，因此从 sing-box 订阅中省略这类已知不可用节点。未知 Xray 版本继续保留旧输出，不猜测。另将 3X-UI 入站更新的 `subSortIndex` 保留条件从正数改成非零，覆盖 3.8.x 新允许的负排序。

本次用 **V3 分支本身**连接真实 3X-UI 3.8.5 / Xray 26.9.9，实机通过完整接口、REALITY 扫描、连接限制、五组流量底线、批量启停、共享客户端迁移和并发写入测试。面板回报 `panelVersion=3.8.5`、`xrayVersion=26.9.9`，REALITY 扫描确认 `CurveID=X25519MLKEM768`。因此只有 v3.9.3+ 的新窄范围将 `max_tested_xui` 提升到 3.8.5；v3.9.1–v3.9.2 继续保持 3.7.0，避免把修复错误地追溯到旧二进制。

同一 V3 分支也在由 1.6.2 数据库升级而来的真实 S-UI 1.6.3 / sing-box 1.14.1 上通过 `TestLive_SUISurface` 和 `TestLive_SUIBulkSetEnabled`。状态、入站和客户端的增删改查、挂载、批量启停及凭据保留均通过；S-UI 的 `/apiv2` 合约未变，不需要生产适配器改动。V3 的 S-UI 已测上限提升到 1.6.3。
...
**改哪个分支(v3.9.4 起变了)**:V3 的 manifest 现在从 **`release/v3`** 拉,也就是你正在
看的这个分支——改这里、push 这里,就够了。v3.9.3 及更早是从 `main` 拉的;而 `main` 现在是
...
- **`main` 上那份不要删**:v3.9.3 及更早的二进制把旧 URL 编译进去了,还在拉它。它现在是给那批
...
- **新 3X-UI 出 patch 版本(无 API 改动)** ── 在当前 active major 的 JSON 里把
  ...
  和 `notes`。commit + push 到 **`release/v3`** → 所有 v3.9.4+ 的部署 60 秒内自动感知
  (v3.9.3 及更早只看 `main` 上那份冻结镜像,要覆盖它们才需要同步过去)。
