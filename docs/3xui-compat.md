...
sing-box 输出仍省略对应 REALITY 节点。跨过版本边界的存量受管 inbound 会由版本探测触发
配置收敛。应使用支持这些选项的客户端并刷新订阅。
...
[v1.6.2](https://github.com/alireza0/s-ui/releases/tag/v1.6.2) 修复设置页保存失败
（[#1258](https://github.com/alireza0/s-ui/commit/3a0759e6e16f3b88ef1050acf6deda24afc9f05f)）：
`GetAllSetting` 返回表单前过滤未知项和迁移标记，数据库中的标记保持不变。
相对 1.6.1，生产代码仅新增这 12 行；其余是回归测试及面板/前端版本更新。
PSP 不使用该设置 API，Token、client/inbound/TLS 接口及 sing-box 1.14.0 均未改变。

使用 S-UI tag `79df629`、固定的 frontend `ab6ed514`、Go 1.26.7、上游 `test`
构建配置、darwin/arm64 启动全新本地面板，PSP checkout `993e305` 的
`TestLive_SUISurface` 和临时 overlay 的凭据、启停、到期、配额、批量生命周期、
服务端计数及维护状态检查全部通过。另验证设置读取后重新保存成功、六个迁移标记仍保留，
并运行上游两个设置回归测试。构建未含 Naive outbound；未验证完整代理流量、配额执法、
已有数据库升级或历史发布二进制。无需修改生产适配器，更新 v3/v4 上限及升级说明，
保留 1.6.0 迁移警告和 1.6.1 登录/维护说明。
