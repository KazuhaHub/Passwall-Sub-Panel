...
- sections 是 allowlist，**v1 恰好是 `host`、`runtime`、`state`、`events` 四个**，每个最多出现一次；
  "最多 8" 是给将来新增留的天花板，校验器只接受上面这四个值——既不放行未知值，也不误拒合法值。
  （2026-09-18 修订：原文只给了一个示例数组和"最多 8"，没有给出集合本身，校验器无从实现。）
- max_events 为 0..200；
...
总结果上限 512 KiB，低于协议 1 MiB 上限留出 envelope 余量。超限时按以下**固定优先级**丢弃
events，并返回 `truncated=true`；不得切断 JSON：

1. **只有 `events` 可以丢。** `host`、`runtime`、`state`、`checks` 一律完整保留——它们是这次
   诊断的结论，丢结论留下细节没有意义。
2. **丢弃顺序：先丢时间最旧的**；时间相同则按 `code` 字典序升序。这样"固定"是可复现的意思，
   不是与时间无关：同一份输入必须得到同一份结果。
3. **逐条丢弃、每次重新编码，直到 ≤ 512 KiB**，不按比例估算后一次性切。
4. **若 events 已全部丢完仍超限，任务以稳定错误码失败**，不得靠切断 JSON 去凑上限。

（2026-09-18 修订：原文写"按固定优先级丢弃最旧 events"，却始终没有给出优先级。丢哪些事件
直接决定运维还能看到什么，属于语义而非实现细节，因此在规格里定死。）

**`events` 的来源**（2026-09-18 修订）：取自 Agent **自持有的有界环形缓冲**里它自己的结构化
事件，即 §14.4 的第一条路线；**不是**日志原文。这里原本有一处冲突：§14 开头声明其条目
"仍需按顺序独立实现，不代表第一阶段一起上线"，而本节把 `events` 写成必填。处理方式是缩小
v1 的范围，而不是替 §14.4 定稿——**v1 只实现"Agent 自有结构化事件"这一条路线**，
