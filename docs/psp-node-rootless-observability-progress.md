...
- §20 场景 D、§23 的 7 天 DoD、数据库增长实测——需要真实时间或完整部署
...
它需要一台真实面板和一台真实 Node 的完整链路，本环境没有。它的判据里"恢复后指标出现
gap""不把整段离线插值成连续线""counter 按最多 15 分钟规则决定是否差分"属于**面板侧**的
派生规则，WP6 的公式测试已经覆盖了那条输入到输出；端到端的那一段要等真实部署。
...
- [ ] §20 场景 D（面板暂时不可达）——需要一台真实面板 + 一台真实 Node 的完整链路；
      它的判据一半落在面板侧差分规则，WP6 的公式测试已覆盖那一半
