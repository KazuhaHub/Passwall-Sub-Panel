...
- [x] WP8 管理 API
- [x] WP9 前端
- [ ] WP10 远程诊断（第二阶段）——**动手前需先补规格，见下节**

## WP10 实现前的规格缺口

WP10（`diagnostics.collect.v1`）在 §13 里定义得比前面几个 WP 薄，有几处必须先回到规格补上。
理由和 WP0、WP1 那两节一样：**本文件与实现都不得替规格做决定**，否则临时选择会变成事实标准。

- [ ] **§13.2 的截断优先级没有定义。** 原文是"超限时按固定优先级丢弃最旧 events"，
      但**优先级本身没给**。丢掉哪些事件直接决定运维还能看到什么，这是语义而非实现细节。
- [ ] **§13.2 的 `events` 依赖 §14.4，而 §14 是候选清单、不是承诺范围。**
      §14 开头写着"每项仍需按顺序独立实现，不代表第一阶段一起上线"；§14.4 给出的是一组
      "可接受路线"（Agent 自持有界环形缓冲 / Core 输出 tee 到脱敏 ring / 默认不上传 /
      上传前须有专门的秘密扫描与测试），不是一份定稿设计。但 §13.2 的 Result 把 `events`
      写成必填。两边需要对齐：要么 §13 首版允许 events 为空，要么 §14.4 先定稿。
- [ ] **§13.1 的 sections allowlist 没有枚举。** 例子给了 `host`/`runtime`/`state`/`events` 四个，
      约束写"最多 8"，但没说这 8 个是哪 8 个。校验器需要确切集合，才能既不放过未知值、
      也不误拒合法值。
- [ ] **"未来另立脱敏规范"尚未存在，但已被两处引用。** §13.3 以它为日志原文禁令的例外，
      §14.4 又要求上传前必须有秘密扫描与测试——两处指向同一件还没写的东西。

**已经明确、可以直接照做的部分**（列出来，是为了把"能做的"和"要补规格的"分开）：
...
**结论**：WP10 不是"接线就完事"的一个 WP——它要新建一个事件子系统，而该子系统的形状在规格里
仍是候选路线；同时截断语义缺一个必需的定义。按本计划一贯的做法，先把上面几条补进规格再开工，
而不是先实现、再让实现去定义契约。
