...
- [x] **真实旧二进制的兼容验证**（§18.7 第 6 条：旧 Node 对新 PSP）——由 CI 的
      ...
      再跑真实的跨仓 live 测试
- [ ] §18.7 第 7 条（**新 Node 对旧 PSP**）——需要一份旧版 PSP。本仓只发布 Node，
      面板侧没有对应的旧版本矩阵；新字段是加成式的，理论上兼容，但没有矩阵可跑
...
## §20 场景 E / F 实测（2026-09-18，同一台 Lima VM）

这两条判据说的是**采集器跨重启报什么**，所以不需要面板，只要一台真实内核的机器。

### 场景 E：Node 重启（同一次开机内跑两个采集进程）

| §20 判据 | 结果 |
|---|---|
| BootID 不变 | ✅ `51d7e902-4364-40…` 两次相同 |
| Agent process counter 断点 | ✅ `processes.agent.counter_epoch` 两次不同（`6d33cb32902b` → `800e126600e0`） |
| Host CPU/network counter 可继续 | ✅ `cpu.system.counter_epoch` 两次相同，`total` 单调增长 |
| uptime 继续增长 | ✅ |
| 不出现负速率 | 这是**面板侧差分**的判据，WP6 的公式测试覆盖输入到输出 |

### 场景 F：宿主机重启（`limactl stop` + `start`）

| §20 判据 | 结果 |
|---|---|
| BootID 变化 | ✅ `51d7e902-4364-40…` → `d6b30a61-94cc-47…` |
| uptime 回落可见 | ✅ 19274690 ms（5h21m）→ 22980 ms（23s） |
| CPU 计数从头起算 | ✅ `cpu.system.total` 重新从 9163 起 |
| 所有 host cumulative counter 断点 | ✅ 见下——**结构上成立**，不是逐个字段测的 |

后一条比看上去强：`internal/host/netstat.go` 的 `counterEpoch(bootID)` 在有 BootID 时
**直接返回它**，因此 cpu/system、network、tcp 的 epoch **就是开机标识**。重启后实测：

```
boot_id    = d6b30a61-94cc-4703-abda-50c1fc2f3229
network    = d6b30a61-94cc-4703-abda-50c1fc2f3229
tcp        = d6b30a61-94cc-4703-abda-50c1fc2f3229
cpu.system = d6b30a61-94cc-4703-abda-50c1fc2f3229
```

也就是说**将来新增一个 boot 级计数器时不可能忘记给它 epoch**——它没有别的来源。
这比"逐个断言每个计数器都断了"更难被绕过。

## §20 场景 D / 首次真实端到端（2026-09-18）

这一条要**一台真实面板 + 一台真实 Node + 一个真实 Core**，所以它顺带成了两个仓库第一次
真正的端到端。本机跑从源码构建的面板（**非默认端口**，见下），VM 里跑 agent。

### 链路是通的

- 面板侧创建 native server → agent_id + credential；
- VM 侧 agent 用该凭据注册，**自己下载并启动了 Xray 26.6.27**；
- 面板记到该 agent 的 `observed_capabilities` =
  `["host.telemetry.v1","task.diagnostics.collect.v1","task.execution.v1","task.expiry.v1"]`
  ——**WP10 的 capability 在两个真实进程之间完成了协商**，这正是此前只能说"没验过"的那一步；
- host 遥测按 60 秒节奏落库。
...
| §20 判据 | 结果 |
|---|---|
| Node 继续本地配额与 Core | ✅ **Xray 一直在跑**（`cores/xray/versions/26.6.27/xray run`），agent 也在跑 |
| sync failure count 增长 | ✅ 连续失败 9 次以上，退避间隔可见（…:38 / …:00 / …:24） |

### 恢复之后

样本序列（`received_at`，UTC）：

```
11:34:44
11:35:53   ← 中断前最后一个
                              （面板 11:36:25 被杀，11:38:56 恢复）
11:39:17   ← 恢复后第一个
11:40:17
```

**中断期间没有任何行**——不是插值出来的连续线，是真的一段空白；图表用
`connectNulls: false`，画出来就是断的。

### 15 分钟差分规则

「恢复后按最多 15 分钟规则决定是否差分」这一条在**面板侧**：`derive.go` 的
`maxDeriveInterval = 15 * time.Minute`，`derive_test.go` 的
`TestDeriveRequiresAMeaningfulPair` 里有一例叫 **"further than the ceiling"**——
把 current 推到一小时之后，`SystemCPUPercent` 必须是 nil，而不是一个跨过空档的速率。
本次停机 2.5 分钟在 15 分钟以内，所以那条分支由单元测试覆盖而不是这次实测。

### 排查时值得知道的一件事

第一次 curl 忘了带 `agent_id`，命中的是 handler 的**第二条** 401 路径
（`report.AgentID != agentID`），不是鉴权失败——**两条路径打印同一个消息**。
