...
>
> **1a 已暂停，且不是因为缺数据。** 为回答「band 该设多大」而去核下限语义时，发现了一个已实机确认的量纲缺陷：PSP 推的是「本期剩余」，面板拿它比「终身累计」，设了配额的用户在用到一半时就会被面板停用并持续翻转。详见 **[traffic-floor-defect.md](traffic-floor-defect.md)**。
>
> **band 是给这个下限值加容差，而这个值当前的量纲本身就是错的**——所以 1a 在缺陷修复前不开工。
