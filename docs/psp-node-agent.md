...
这 21 个方法里有相当一部分**是被 3X-UI 的模型逼出来的**：客户端按 email 全面板唯一、inbound 挂载是一张 junction、整结构 Save 语义、批量接口是为了少触发 xray reload。

于是有个真问题：**agent 照着这个形状实现，等于把 3X-UI 的模型搬进我们自己的后端。**

- 照实现 → ADR 0024「其它什么都不用改」成立，但那些为阻抗失配而生的变通会永久留在协议里；
- 不照实现 → PSP 的 service 层要动，「新增一个 PanelKind」这个卖点就打折。

**这个问题上游调研答不了，它是 PSP 侧的问题。** 建议的解法方向（待定）：agent 实现完整的 21 个方法以保住 drop-in 性质，但**语义按我们的模型定义**（例如 `UpdateClient` 是真正的部分更新，而不是 3X-UI 那种整体替换），并用 `Capabilities()` 声明差异。**待 §7 定稿时一并回答。**
