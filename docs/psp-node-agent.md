...
**解法不是翻转拨号方向。** 三个档位，成本递增：

1. **`next_poll_s`（协议里已经有）。** PSP 在响应里说「2 秒后再来」。
   它**治不了第一次通知**——PSP 学到新配置时节点刚走,只能等下一轮才有机会说话。
   但它把多步交互从 `N×T` 压成 `T + (N-1)×2s`:装 core 那种流程从 8 分钟变成 2 分钟出头。
   **有用，但不够。**
2. **长轮询：节点拨出后 PSP 把请求挂住，有话要说才回。← 这是解法。**
   下发延迟降到接近 0,而**节点拨出买到的每一样都还在**:不需要公网 IP、不需要入站端口、不需要证书、
   NAT 后面也能跑。代价是每台 agent 一条常驻连接（5 台，可忽略）、要处理空闲超时与重连、
   路径上的反代/LB 要容忍挂起请求。§8.5 说过长轮询「买到的是延迟不是顺序」——**延迟正是这里要买的东西**,
   顺序另有版本闸负责。
3. **翻转回 PSP 拨入。** 能解决延迟，但要把节点拨出的**全部**收益还回去（公网可达 + 每台节点一张证书 +
   NAT 后面的节点直接做不了），换来的东西第 2 档用低得多的代价就买到了。**不划算，不要走这条。**

**协议不用改一个字节**——端点还是 `POST /v1/node/sync`,挂起是**传输行为**,不是新的消息。
但 **agent 的同步循环必须从第一天就按「服务端可能挂住我 N 秒」来写**,不能先写成纯轮询以后再改：
轮询和长轮询下,超时、重连、心跳三者的关系不一样（纯轮询里「请求超时」= 故障；长轮询里它是**正常**的），
后期改要动 agent 的核心循环，并且会连带改动失联判定的阈值。**这是 B2 的验收判据之一。**

**一件长轮询也治不了的事**：节点离线时任何拨号方向都送不到。区别只在**谁先知道**——
