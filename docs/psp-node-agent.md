...
- **建议等协议定稿再建仓库。** 一旦 PSP `import` 了它的 module path，改名和改路径的成本就上来了。现在建一个空仓库买不到任何东西，却要提前锁死名字。
  ...
  > **2026-09-09：这个前提已经满足**（§8 定稿）。命名判据按上面那条——**按它是什么命名**：一个由外部控制面驱动、负责跑 xray / sing-box 并上报的节点后端。
  >
  > **建议名：`coreplane`**（module path `github.com/KazuhaHub/coreplane`）。它把该说的两件事都说了：它**管一个 core**（xray / sing-box），它是**控制面的节点那一半**，而两者都不提 PSP。生态里的邻居是 XrayR / V2bX / soga / Marzban-node，`coreplane` 与它们都不撞。
  >
  > 备选：`nodeplane`（更强调「节点侧」，更弱地表达「管 core」）、`corewarden`（更长）。
  >
  > **这一条等所有者拍板再落地**——它是这个项目的公开身份，不该由实现顺手定下。
...
- **仓库名**：建议 `coreplane`，等所有者拍板（§0.5）

## 10. 下一步

按依赖顺序，不是按难度：

1. **拍板仓库名**（一句话的事，但它是 module path，`import` 之后改就贵了）。
