...
**解法不是翻转拨号方向，而是把周期缩短。** 三个档位：

1. **快轮询：把 T 从 120s 降到 5s。← 选这个。**
   5 台 agent × 每 5 秒 = **1 req/s**,PSP 和中间的 CDN 都感觉不到。延迟 ≤5s,
   与今天 `provisionNodeMembers` 写下的「within seconds」**等价**——原目标就达成了，不欠账。
   HTTP keep-alive 让 TCP/TLS 连接在多次轮询之间复用，所以并不是每 5 秒握一次手；
   贵的从来是**建立**连接，不是**保持**。
2. **长轮询：PSP 把请求挂住，有话说才回。← 备选，不是首选。**
   延迟接近 0,但**挂起请求会撞上路径上的反向代理**。本项目的部署路径上有 Cloudflare，
   其源站响应超时约 100 秒，超过即 524。挂 60s 技术上能过，但这是**在别人的超时阈值边上跑**——
   为了把 5 秒压到 0 秒买一个长期的运维约束，不划算。**留作后手**：如果哪天出现真正需要亚秒下发的
   需求（例如实时封禁），再启用，那时协议不用改，只改传输行为。
3. **翻转回 PSP 拨入。** 能解决延迟，但要把节点拨出的**全部**收益还回去（公网可达 + 每台节点一张证书 +
   NAT 后面的节点直接做不了）。第 1 档零成本就够用。**不划算，不要走这条。**

**周期由 `next_poll_s` 驱动，不写死。** PSP 忙时说「1 秒后再来」,闲时说「30 秒后再来」,
节点只管照办。这个字段协议里已经有,不用新增。它同时把多步交互从 `N×T` 压成 `T + (N-1)×1s`。

**代价（唯一一笔，必须付）：轮询节奏与报告节奏必须解耦。** 每 5 秒发一次全量逐客户端计数枚举是浪费。
所以 `NodeReport` 增加一个 `partial` 标记：轻量轮询只带 `have`（三个流各自持有什么）与 `issues`,
省掉 `objects` / `clients` / `subjects`;全量报告仍按 120s 的节奏发。
...
**而且省下的东西本来就不花钱**：5 秒轮询是 1 req/s。这是在为一个不存在的成本做优化，
代价是换来一个触发不了的触发器。
