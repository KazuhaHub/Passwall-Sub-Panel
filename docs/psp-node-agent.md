...
- **状态**：**协议与 Xray 生产路径已实现（2026-09-11）**——六个洞、三流契约、PSP 生产路由、
  原生节点凭据生命周期、Node daemon、遥测、离线执法和发布矩阵均已闭合。后续范围见 §9。
...
| `LiveIPReader` | `ListLiveClientIPs()` | **必须**——异地并发检测依赖它。只实现它时没有时间戳，PSP 把返回的每个地址都当作此刻在连（窗口 = 并发）；能给出每个地址的节点与最后看到的时间，就另实现 `LiveIPDetailReader.ListLiveClientIPDetails()`，见 `connection-limits.md` §12.4.3 |
...
**端点**：稳态唯一 `POST /v1/node/sync`。节点拨出，上行 `NodeReport`、下行三段，同一次往返——响应必须在知道 agent 刚报了什么的前提下计算。§2.1 的五个**调用**（RealityProbe / CoreVersions / CoreInstall / AgentUpgrade / TLSMaterial）转成响应里的 `tasks[]` + 下一轮的 `task_results[]`。**这笔账明着付**（ADR 0025 Q0：「这是要主动付的代价，不是换个拨号方就白得的」）：交互延迟最长一个心跳周期，响应带 `next_poll_seconds` 供 PSP 下调。**这句只记账，不结账——延迟怎么真正付掉见 §8.7（长轮询）。**
...
> **可预知到期已经有单一权威链，不再是协议洞**：
> `User.PushExpireTime → PSPClient.DesiredExpiryTime → Client.ExpiresAtMS`。B3 runtime 收到 roster
> 后必须持有这个绝对截止时间，并在 PSP 断线时仍本地停用；不能靠未来 roster 条目缺席来触发。
> 真正不可预知的是**断线之后才发生**的管理员停用或策略撤销。任何单通道、无租约协议都无法
> 提前执行一个尚未发生的决定；要缩短该窗口只能引入租约或第二通道，这是 §9 的生产取舍。
...
1. **PSP 自己今天对并发地址的「个数」一个动作都没有。** 异地并发检测默认只观测（`connection-limits.md` §12.5）；它唯一的自动动作 `geo_auto` 默认关闭、按分组开启、有时限，判的是一个人**同时在几个地方**，不是他用了几个地址——任务 #49（被标记的账号怎么处理）有了这个可选的答案，但它不涉及个数。节点执行一个地址个数上限，就不是「精确执法之下的一层网」,**它就是策略本身**——而这条策略从未被批准。
...
`computed_at_ms` / `numerator_as_of_ms` / `numerator_oldest_report_age_ms` / `next_poll_seconds` **写进响应信封，不进段体、不进 ETag、不触发重铸**——这是 §8.3「时间戳不进 ETag」+「铸造必须内容幂等」两条的直接后果。写进段体等于把 99% 的 skip 在协议层抵消掉。

**新鲜度只用 PSP 的接收时钟。** `reported_at_ms` 是 agent 自报值，只用于时钟偏差诊断；拿它算
`entries_stale` 或授权分子年龄会让快跳时钟把离线报告伪装成永久新鲜。PSP 缓存每份全量报告时
同时记录本机接收时间，按 Node 的离散调度计算实际最晚全量周期（例如 poll=30s、full=45s，
实际是 60s；full=0 表示每轮全量，实际周期就是 poll），过期即置 `want_full_report=true`，并在
新全量报告到达前不拿旧计数给有限额客户端发放新 headroom。
...
### 8.6 落地前置条件（2026-09-09 状态）

1. **期望文档必须只有一个铸造者**（ADR 0025 债务 1b，**已完成**）。`clientdoc` 铸造并持久化唯一 desired 定义，legacy 写入与 native roster 都消费同一份字段。
2. **客户端行身份必须稳定**（债务 1c，**已于 2026-09-09 完成**）—— 这是 `cli_{psp_clients.id}` 成立的唯一条件。`psp_clients.id` 现在是唯一稳定身份，email 已从唯一键降级为可变属性；planner 按旧/新挂载最大重叠延续行，1↔2 分区与域名重算不再重置 id 或计数基线。
3. **落库位置（已完成）**：`node_agents` + 每流状态行已建立；挂载是四态、应用版本、失败时长与最后确认凭据快照。Node 产生的 Issue 进入 PSP 独立收件箱并可人工确认查看。
4. **版本差不能当存活信号（已完成）**：`last_seen` 独立落库，`OfflineAt` 只读它、不读版本。
   三个心跳未见即离线；全量观测另以“有效全量周期 + 一个心跳”作为读面宽限。PSP 只用自己的
   receipt time 判定，agent 自报时间只用于诊断，时钟回退按未知/过期处理。
5. **§4 的成本表（已完成）**：已按 5 台 agent、合计 9 个监听器、T=120s 重算，并把 94% 的收益归因于拨号方向翻转而非文档分段。

### 8.7 下发延迟：拨出不等于慢（2026-09-09 定）

§8.3 说「交互延迟最长一个心跳周期」并给了 `next_poll_seconds`,那是**把账记下来**,不是**把账付掉**。
这一节付掉它。
...
**它欠下的那点延迟，由 `next_poll_seconds` 还，而不是由基线还：**

- **多步交互**（探测 → 取结果 → 装 core → 确认）:PSP 一旦手上有排队的活，就在响应里说
  「1 秒后再来」。于是代价从 `N×30s` 降到 `30s + (N-1)×1s`。协议里这个字段已经有。
- **头一跳仍然要付满一个基线**——PSP 学到变更时节点刚走，这是 `next_poll_seconds` 治不了的。
  对此 PSP 可以开**热窗**:管理员正在某个节点的页面上操作、或某个用户刚拉过订阅时，
  把那台 agent 的 `next_poll_seconds` 压到 2 秒并维持两分钟。管理员点下按钮时那台 agent 早已在快轮询。
...
发请求、拿响应、按 `next_poll_seconds` 睡一会儿。「请求超时 = 故障」这个直觉在快轮询下**是对的**,
...
但那笔成本的正确解法是**把基线调稀 + 让 PSP 用 `next_poll_seconds` 在需要时加速**,
