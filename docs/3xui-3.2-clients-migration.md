...
> **P2(live 写验证)/ P3(reconcile 轴 A inbound 回推)待真实 3.2.0 面板验证后再做。**
...
- **P2 — live 写验证**(§9):在真实 3.2.0 面板的临时 client 上验 `/clients/add` 的字段接受度
  (id/flow/password/method/auth/subId 是否都吃)、`/clients/update` 整行替换是否清掉 `subId`、
  以及 §4.3 的 inbound-update 是否覆盖 clients 表。beta rollout 时一并验。
- **P3 — reconcile 轴 A**(§4.3):若 P2 验出 `/inbounds/update` 重注入 `settings.clients`
  会被 clients 表覆盖 / 产生孤儿,改 `reconcileInboundConfig` 为只推非 client 的 inbound 配置。
- **P5(可选)— bulk client 端点**:把批量启停 / 批量删 / 删用户全部 client 接 `/clients/bulk*`,
  N 次 HTTP 降到 1 次。增量优化,与兼容性修复解耦。
- **文档同步**:`docs/ARCHITECTURE.md` 的 adapter API 清单(§3X-UI client)仍列着旧的
  `DelClient` / `CopyClients` / `GetClientTraffic` / `GetInboundTraffics` / `ResetClientTraffic`,
  待一次 docs pass 同步到 `/clients/*`。
