...
2. 在 mysql adapter 加实现 + 测试:
   - `internal/adapters/sqlstore/user_repo.go` + `user_repo_test.go`
   - `internal/adapters/sqlstore/ownership_repo.go` + `ownership_repo_test.go`
   - `internal/adapters/sqlstore/traffic_repo.go` + `traffic_repo_test.go`
