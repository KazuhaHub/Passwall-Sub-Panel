...
V3 稳定维护线与 V4 测试线使用独立滚动通道：

| Tag | 跟踪 | 适合 |
|---|---|---|
| `:latest` | 最新**稳定版**（默认） | 生产 / 日常 |
| `:v3` | 最新 V3 稳定维护版 | 继续使用 V3 数据模型 |
| `:beta` | V4 测试版 | 提前测试 V4 |
| `:v3.9.3` / `:v4.0.0-beta.10` | 钉死某个确切版本 | 不想自动滚动 |
...
> V3 维护版只更新 `:latest` 与 `:v3`，不会改动 `:beta`，因此发布 V3 安全修复时不会把 V4 测试部署降回 V3。想钉死不动请使用完整版本号 `:vX.Y.Z`。
>
> **想长期留在 V3，请用 `:v3` 而不是 `:latest`。** `:latest` 的含义是「本仓库最新的稳定版」，不是「最新的 V3」——现在两者恰好相同，只因为 V4 还全部是测试版。等 V4 发布正式版，`:latest` 就会指向 V4，届时跟着 `:latest` 的部署会被带过大版本边界（V4 有不同的数据库模型）。`:v3` 永远只跟 V3 稳定维护版，不会有这个问题。
>
